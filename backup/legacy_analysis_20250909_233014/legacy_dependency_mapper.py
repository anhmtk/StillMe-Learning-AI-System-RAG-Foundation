...
class LegacyDependencyMapper:
    ...
    def create_dependency_map(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "graph_stats": {
                ...
                "legacy_nodes": len([n for n in self.dependency_graph.nodes() if n.startswith("legacy_")]),
                "current_nodes": len([n for n in self.dependency_graph.nodes() if n.startswith("current_")])
            }
        }
    ...
    def _create_migration_mappings(self) -> List[Dict[str, Any]]:
        ...
        potential_mappings = {
            ...
            "smart_g_p_t__a_p_i__manager": ["unified_api_manager", "api_provider_manager"],
        ...
        for legacy_module in self.legacy_data["modules"]:
            ...
            mapping = {
                ...
                "migration_complexity": self._assess_migration_complexity(legacy_module),
                "recommendations": self._generate_migration_recommendations(legacy_module, existing_targets)
            }
    ...
    def _generate_migration_recommendations(self, legacy_module: Dict[str, Any], targets: List[str]) -> List[str]:
        """Generate migration recommendations"""
        recommendations = []
        
        if not targets:
            recommendations.append("No direct equivalent found - consider creating new module")
            recommendations.append("Review functionality and integrate into existing modules")
        else:
    ...
    def generate_visualization(self, output_path: str = "backup/legacy_analysis_20250909_233014/dependency_map.png"):
        ...
        try:
            ...
            # Color nodes by type
            legacy_nodes = [n for n in self.dependency_graph.nodes() if n.startswith("legacy_")]
            current_nodes = [n for n in self.dependency_graph.nodes() if n.startswith("current_")]
...
def main():
    ...
    try:
        ...
        # Save dependency map
        map_path = Path("backup/legacy_analysis_20250909_233014/legacy_dependency_map.json")
        with open(map_path, 'w', encoding='utf-8') as f:
            json.dump(dependency_map, f, indent=2, ensure_ascii=False)
        
        print(f"✅ Dependency mapping complete! Map saved to: {map_path}")
        print(f"📊 Found {len(dependency_map['migration_mappings'])} migration mappings")
        print(f"🔍 Analyzed {dependency_map['graph_stats']['total_nodes']} total modules")
