...
class LegacyCodeAnalyzer:
    ...
    def _identify_patterns(self):
        ...
        for anti_pattern in all_anti_patterns:
            anti_pattern_counts[anti_pattern] = anti_pattern_counts.get(anti_pattern, 0) + 1
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if any("uses_print_instead_of_logging" in module.anti_patterns for module in self.modules):
...
def main():
    ...
    try:
        ...
        # Save report
        report_path = Path("backup/legacy_analysis_20250909_233014/legacy_analysis_report.json")
        with open(report_path, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        print(f"✅ Analysis complete! Report saved to: {report_path}")
        print(f"📊 Analyzed {report['total_modules']} legacy modules")
        print(f"🔍 Found {len(report['pattern_analysis']['common_patterns'])} common patterns")
        print(f"⚠️ Found {len(report['pattern_analysis']['anti_patterns'])} anti-patterns")
