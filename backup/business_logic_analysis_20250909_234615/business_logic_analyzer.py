...
class BusinessLogicAnalyzer:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.human_input_indicators = {
            ...
            "configuration": [
                r"config.*required",
                r"setup.*needed",
                r"initialization.*required",
                r"environment.*variable",
                r"api.*key.*required",
                r"credentials.*needed",
            ],
        }
    ...
    def _extract_business_rules_from_content(
        ...
        for rule_type, rules in rule_groups.items():
            if rules:
                pattern = DecisionPattern(
                    ...
                    automation_feasibility=self._calculate_average_automation_potential(
                        rules
                    ),
    ...
    def _calculate_average_complexity(
        ...
        avg_value = sum(complexity_values[rule.complexity] for rule in rules) / len(
            rules
        )
    ...
    def _calculate_average_automation_potential(
        self, rules: List[BusinessRule]
    ) -> float:
    ...
    def _generate_automation_recommendations(self) -> List[Dict[str, Any]]:
        ...
        high_potential_rules = [
            rule for rule in self.business_rules if rule.automation_potential > 0.7
        ]
        ...
        low_potential_rules = [
            rule for rule in self.business_rules if rule.automation_potential < 0.4
        ]
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        avg_automation_potential = (
            sum(rule.automation_potential for rule in self.business_rules) / total_rules
        )
        ...
        return {
            ...
            "high_priority_rules": len(
                [
                    ...
                    if r.complexity == ComplexityLevel.CRITICAL
                ]
            ),
            "automation_ready_rules": len(
                [r for r in self.business_rules if r.automation_potential > 0.7]
            ),
        }
...
def main():
    ...
    try:
        ...
        result_path = Path(
            "backup/business_logic_analysis_20250909_234615/business_logic_analysis.json"
        )
        ...
        print(
            f"🔍 Identified {len(analysis_result['decision_patterns'])} decision patterns"
        )
        print(
            f"📈 Average automation potential: {analysis_result['summary']['automation_potential']}"
        )
