...
class ModelWarmupManager:
    ...
    def __init__(self, config_file: str = "config/model_warmup_config.json"):
        ...
        # Keep-alive settings
        self.heartbeat_interval = self.config.get("heartbeat_interval_seconds", 300)  # 5 minutes
    ...
    def preload_models(self) -> Dict[str, bool]:
        ...
        self.logger.info(f"🎯 Pre-loading completed: {sum(results.values())}/{len(results)} models ready")
        return results
    ...
    def _warmup_model(self, model: str) -> bool:
        ...
        try:
            ...
            for chunk in call_ollama_chat(model, messages, stream=False, options=options, timeout=self.warmup_timeout):
            ...
            if response_received:
                ...
                total_time = self.warmup_stats["avg_warmup_time_ms"] * (self.warmup_stats["successful_warmups"] - 1)
                self.warmup_stats["avg_warmup_time_ms"] = (total_time + warmup_time) / self.warmup_stats["successful_warmups"]
    ...
    def _keep_alive_worker(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                with self._lock:
                    warm_models = [name for name, status in self.model_status.items() if status.is_warm]
    ...
    def _send_heartbeat(self, model: str):
        ...
        try:
            ...
            # Send heartbeat
            response_received = False
            for chunk in call_ollama_chat(model, messages, stream=False, options=options, timeout=10):
                if chunk and 'response' in chunk:
                    response_received = True
                    break
            ...
            with self._lock:
                if model in self.model_status:
                    ...
                    if self.model_status[model].error_count >= 3:
                        self.model_status[model].is_warm = False
                        self.logger.warning(f"⚠️ {model} marked as not warm due to repeated errors")
    ...
    def get_warm_models(self) -> List[str]:
        ...
        with self._lock:
            return [name for name, status in self.model_status.items() if status.is_warm]
    ...
    def get_performance_stats(self) -> Dict:
        ...
        with self._lock:
            return {
                ...
                "warm_model_count": len([s for s in self.model_status.values() if s.is_warm]),
        ...
        try:
            ...
            performance_data = {
                ...
                "model_status": {name: {
                    ...
                    "warmup_time": status.warmup_time.isoformat() if status.warmup_time else None,
