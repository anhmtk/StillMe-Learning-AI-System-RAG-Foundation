...
def test_feedback_summary():
    ...
    feedback_samples = [
        "Great performance, very fast! - positive experience",
        "The system is too slow",
        "Good quality output - positive experience",
        "The interface is confusing",
        "Excellent reliability - positive experience",
    ]

    for i, feedback in enumerate(feedback_samples):
        engine.record_feedback(
            user_id="test_user", feedback_text=feedback, session_id=f"test_session_{i}"
        )
...
def test_suggestion_rules():
    ...
    performance_suggestions = [
        s for s in suggestions if "performance" in s.lower() or "optimiz" in s.lower()
    ]
    assert (
        len(performance_suggestions) > 0 or len(suggestions) == 0
    )  # Either has suggestions or empty
...
def test_multiple_users():
    ...
    feedbacks = ["Great performance!", "The system is too slow", "Good quality output"]

    for user, feedback in zip(users, feedbacks, strict=False):
        engine.record_feedback(
            user_id=user, feedback_text=feedback, session_id=f"session_{user}"
        )
...
def test_empty_feedback():
    ...
    # Test empty feedback
    success = engine.record_feedback(
        user_id="test_user", feedback_text="", session_id="test_session"
    )
    assert success is True
