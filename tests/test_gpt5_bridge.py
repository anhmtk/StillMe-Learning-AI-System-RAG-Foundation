...
async def test_bridge_fast_fallback_ollama(monkeypatch, app_with_lifespan: FastAPI):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        r = await ac.post("/dev-agent/bridge", json={"prompt":"hello", "mode":"fast", "provider":"auto"})
...
async def test_bridge_safe_runs_saferunner(monkeypatch, app_with_lifespan: FastAPI):
    ...
    transport = ASGITransport(app=app_with_lifespan)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        r = await ac.post("/dev-agent/bridge", json={"prompt":"do safe thing", "mode":"safe", "provider":"auto"})
        assert r.status_code == 200
        data = r.json()
