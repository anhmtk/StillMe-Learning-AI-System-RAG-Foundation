        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing_fields and health_data["status"] == "healthy":
                    passed += 1
                    print("PASSED: Health endpoint returns valid data")
                else:
        ...
        try:
            ...
            if dashboard_path.exists():
                ...
                if not missing_fields and len(dashboard.get("panels", [])) > 0:
                    passed += 1
                    print("PASSED: Grafana dashboard configuration valid")
                else:
                    print(f"FAILED: Missing fields or no panels: {missing_fields}")

            else:
                print("FAILED: Grafana dashboard file not found")
        ...
        try:
            ...
            if alerts_path.exists():
                ...
                if not missing_alerts:
                    passed += 1
                    print("PASSED: All required alert rules present")
                else:
                    print(f"FAILED: Missing alert rules: {missing_alerts}")

            else:
                print("FAILED: Alert rules file not found")
