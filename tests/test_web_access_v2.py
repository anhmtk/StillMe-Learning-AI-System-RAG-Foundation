...
class TestWebToolsRegistry:
    ...
    async def test_search_news_success(self, web_tools):
        ...
        result = await web_tools.call_tool(
            "web.search_news", query="AI technology", window="24h"
        )
    ...
    async def test_github_trending_success(self, web_tools):
        ...
        result = await web_tools.call_tool(
            "web.github_trending", topic="python", since="daily"
        )
    ...
    async def test_google_trends_success(self, web_tools):
        ...
        result = await web_tools.call_tool(
            "web.google_trends", terms=["AI", "ML"], region="VN", days=7
        )
...
class TestContentWrapSecurity:
    ...
    def test_validate_wrapped_content(self, content_wrap):
        ...
        is_valid, reason = content_wrap.validate_wrapped_content(
...
class TestWebMetrics:
    ...
    def test_cache_hit_ratio(self, metrics):
        ...
        metrics.record_request(
            "web.search_news", True, 100.0, True, "newsapi.org", None, 1024
        )
        metrics.record_request(
            "web.search_news", True, 200.0, True, "newsapi.org", None, 1024
        )
        metrics.record_request(
            "web.search_news", True, 1500.0, False, "newsapi.org", None, 1024
        )
    ...
    def test_get_summary(self, metrics):
        ...
        metrics.record_request(
            "web.github_trending", True, 3000.0, False, "api.github.com", None, 4096
        )
    ...
    def test_performance_alerts(self, metrics):
        ...
        for _i in range(5):
            metrics.record_request(
                "web.search_news", False, 10000.0, False, "newsapi.org", "timeout", 0
            )
...
class TestSandboxController:
    ...
    def test_redirect_limit(self, sandbox):
        ...
        result = sandbox.is_egress_allowed(
            "https://api.github.com/test", redirect_count=5
        )
        assert result["allowed"] is False
...
class TestRedTeamScenarios:
    ...
    def test_prompt_injection_attempts(self, tool_gate):
        ...
        for attempt in injection_attempts:
            decision = validate_tool_request(
                "web.search_news", {"query": "test"}, attempt
            )
            assert (
                decision.allowed is False
            ), f"Injection attempt not blocked: {attempt}"
    ...
    def test_xss_attempts(self, content_wrap):
        ...
        for attempt in xss_attempts:
            ...
            assert (
                wrapped.injection_detected is True
            ), f"XSS attempt not detected: {attempt}"
            assert (
                "[REMOVED_" in wrapped.wrapped_content
            ), f"XSS not sanitized: {attempt}"
    ...
    def test_obfuscation_attempts(self, content_wrap):
        ...
        for attempt in obfuscation_attempts:
            ...
            assert (
                wrapped.injection_detected is True
            ), f"Obfuscation not detected: {attempt}"
    ...
    def test_redirect_attacks(self, sandbox):
        ...
        result = sandbox.is_egress_allowed(
            "https://api.github.com/test", redirect_count=10
        )
    ...
    def test_parameter_injection(self, tool_gate):
        ...
        for params in injection_params:
            ...
            assert (
                decision.allowed is False
            ), f"Parameter injection not blocked: {params}"
...
class TestIntegrationScenarios:
    ...
    async def test_full_web_request_flow(self):
        ...
        assert (
            result["allowed"] is True or result["allowed"] is False
        )  # Depends on allowlist
