...
def test_jailbreak_vi():
    ...
    assert result.get('reason', '').startswith('fallback_template') or result.get('reason', '').startswith('model_ok')
...
def test_unicode_en():
    ...
    # Check text contains required tokens
    text = result.get('text', '')
    assert 'recognized special characters' in text, f"Expected 'recognized special characters' in text: {text}"
...
def test_injection_en():
    ...
    # Check classification
    assert result.get('reason', '').startswith('policy:injection') or result.get('reason', '').startswith('policy_block')
    
    # Check blocked
    assert result.get('blocked', False) == True, f"Expected blocked=True, got: {result.get('blocked')}"
