...
class TestDecisionEngine:
    ...
    def test_decision_engine_criteria_calculation(self):
        ...
        security_score = self.decision_engine._calculate_security_score(
            option_data, context
        )
        assert 0.0 <= security_score <= 1.0

        performance_score = self.decision_engine._calculate_performance_score(
            option_data, context
        )
        assert 0.0 <= performance_score <= 1.0

        maintainability_score = self.decision_engine._calculate_maintainability_score(
            option_data, context
        )
    ...
    def test_decision_engine_risk_assessment(self):
        ...
        criteria_scores = {"security": 0.8, "performance": 0.7, "maintainability": 0.6}

        risk_level = self.decision_engine._assess_risk_level(
            option_data, criteria_scores
        )
...
class TestExperienceMemory:
    ...
    def test_store_experience(self):
        ...
        outcome = {"success": True, "issues_found": 2, "time_taken": 30}
    ...
    def test_experience_query(self):
        ...
        for i in range(5):
            ...
            self.experience_memory.store_experience(
                ...
                (
                    ExperienceCategory.SECURITY
                    if i % 2 == 0
                    else ExperienceCategory.PERFORMANCE
                ),
                context,
                f"Test action {i}",
                outcome,
        ...
        query = ExperienceQuery(
            categories=[ExperienceCategory.SECURITY], success_only=True, limit=10
        )
...
class TestSafeAttackSimulator:
    ...
    def test_simulation_history(self):
        ...
        assert latest_simulation.status in [
...
class TestIntegration:
    ...
    def test_end_to_end_decision_process(self):
        ...
        assert decision_result.implementation_status in [
            DecisionStatus.APPROVED,
            DecisionStatus.REJECTED,
        ]
        ...
        if decision_result.implementation_status == DecisionStatus.APPROVED:
            experience_id = self.experience_memory.store_experience(
                ...
                True,
                [
                    "Security improvements are important",
        ...
        assert simulation_result.status in [
            SimulationStatus.COMPLETED,
            SimulationStatus.FAILED,
        ]
    ...
    def test_learning_from_experiences(self):
        ...
        assert (
            len(self.experience_memory.patterns) >= 0
        )  # Patterns may or may not be created
    ...
    def test_comprehensive_security_assessment(self):
        ...
        total_vulnerabilities = sum(
            len(r.vulnerabilities_found) for r in simulation_results
        )
        total_defenses = sum(len(r.defenses_triggered) for r in simulation_results)
        average_risk_score = sum(r.risk_score for r in simulation_results) / len(
            simulation_results
        )
        ...
        assessment_outcome = {
            ...
            "scenarios_completed": len(
                [
                    ...
                    if r.status == SimulationStatus.COMPLETED
                ]
            ),
        }
