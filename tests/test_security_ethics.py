...
from conftest import (
    test_config, test_datasets, mock_ethics_system, mock_content_filter,
    mock_secure_memory, mock_conversational_core, mock_api_manager,
    test_utils
)
...
class TestPromptInjectionAttacks:
    ...
    def test_instruction_override_attack(self, mock_ethics_system, mock_content_filter, test_datasets):
        """Test detection of instruction override attacks."""
        redteam_data = test_datasets.get('redteam_prompts', {})
        injection_cases = [case for case in redteam_data.get('redteam_prompts', []) 
                          if case.get('category') == 'jailbreak' and 
                          case.get('attack_type') == 'instruction_override']
        ...
        for case in injection_cases:
            ...
            filter_response = {"is_safe": False, "filtered_content": "Blocked: instruction override"}
    ...
    @pytest.mark.security
    def test_role_switching_attack(self, mock_ethics_system, mock_content_filter, test_datasets):
        """Test detection of role switching attacks."""
        redteam_data = test_datasets.get('redteam_prompts', {})
        role_switching_cases = [case for case in redteam_data.get('redteam_prompts', []) 
        ...
        for case in role_switching_cases:
            ...
            filter_response = {"is_safe": False, "filtered_content": "Blocked: role switching"}
            mock_content_filter.filter.return_value = filter_response
...
class TestPIIRedaction:
    ...
    def test_email_redaction(self, mock_content_filter, test_datasets):
        ...
        email_cases = [case for case in pii_data.get('pii_samples', []) 
    ...
    def test_phone_number_redaction(self, mock_content_filter, test_datasets):
        ...
        pii_data = test_datasets.get('pii_samples', {})
        phone_cases = [case for case in pii_data.get('pii_samples', []) 
...
class TestContentSafetyValidation:
    ...
    def test_harmful_content_detection(self, mock_ethics_system, test_datasets):
        ...
        redteam_data = test_datasets.get('redteam_prompts', {})
        harmful_cases = [case for case in redteam_data.get('redteam_prompts', []) 
