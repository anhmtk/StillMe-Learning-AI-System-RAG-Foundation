...
def test_short_term_search(memory_system):
    ...
    assert any("coffee" in memory_system.short_term._decrypt(item.content).lower() for item in results)
...
def test_mid_term_compression(memory_system, sample_item):
    ...
    for i in range(10):
        memory_system.mid_term.add(MemoryItem(
            content=f"Item {i}",
            priority=0.5,
            timestamp=datetime.now(),
            last_accessed=datetime.now(),
...
def test_search_case_insensitive(memory_system):
    ...
    assert any("mixed" in memory_system.short_term._decrypt(item.content).lower() for item in results)
