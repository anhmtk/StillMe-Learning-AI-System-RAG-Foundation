...
from emotionsense_v1 import EmotionSenseV1, EMOTION_CATEGORIES, ERROR_CODES
...
class TestEmotionSenseV1(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_cases_vietnamese = [
            ...
            {"text": "Tôi đang rất buồn và thất vọng", "expected": "sad", "confidence": 0.8},
            {"text": "Mày làm tao tức điên lên rồi!", "expected": "angry", "confidence": 0.9},
            {"text": "Tôi sợ không dám làm đâu", "expected": "fear", "confidence": 0.7},
            {"text": "Trời ơi bất ngờ quá!", "expected": "surprise", "confidence": 0.75},
            {"text": "Tôi muốn biết thông tin này", "expected": "neutral", "confidence": 0.6},
            {"text": "Tôi không hiểu cái này là sao", "expected": "confused", "confidence": 0.65}
        ]
        
        # Test cases edge cases theo yêu cầu
        self.test_cases_edge = [
            {"text": "", "expected": "neutral", "confidence": 0.0},  # Empty text
            {"text": "123456", "expected": "neutral", "confidence": 0.0},  # Numbers only
        ...
        self.test_cases_english = [
            ...
            {"text": "I feel sad and disappointed", "expected": "sad", "confidence": 0.8},
            {"text": "I am so angry with you!", "expected": "angry", "confidence": 0.8},
            {"text": "I am afraid to do this", "expected": "fear", "confidence": 0.7},
            {"text": "Oh my god, this is amazing!", "expected": "surprise", "confidence": 0.75},
            {"text": "I need to know this information", "expected": "neutral", "confidence": 0.6},
            {"text": "I don't understand what this means", "expected": "confused", "confidence": 0.65}
        ]
    ...
    def test_2_vietnamese_emotion_detection(self):
        ...
        for i, test_case in enumerate(self.test_cases_vietnamese, 1):
            ...
            try:
                # Detect emotion
                result = self.emotion_sense.detect_emotion(text, language="vi", user_id=self.test_user_id)
                ...
                # Log result
                status = "✅ PASS" if emotion_detected == expected_emotion else "❌ FAIL"
                print(f"    {i:2d}. '{text[:30]:<30}' → {emotion_detected} (conf: {confidence:.2f}) [{status}]")
                
                # Assertions
                self.assertEqual(language, "vi", f"Language should be 'vi', got {language}")
        ...
        print(f"    ✅ Vietnamese emotion detection: {passed_tests}/{total_tests} passed ({success_rate:.1f}%)")
        
        # Quality target: >85% accuracy on Vietnamese testset
        self.assertGreaterEqual(success_rate, 85.0, f"Vietnamese accuracy {success_rate:.1f}% below target 85%")
    ...
    def test_3_english_emotion_detection(self):
        ...
        for i, test_case in enumerate(self.test_cases_english, 1):
            ...
            try:
                # Detect emotion
                result = self.emotion_sense.detect_emotion(text, language="en", user_id=self.test_user_id)
                ...
                # Assertions
                self.assertEqual(language, "en", f"Language should be 'en', got {language}")
                self.assertTrue(result["success"], "Result should be successful")
                self.assertGreaterEqual(confidence, 0.0, "Confidence should be >= 0")
                self.assertLessEqual(confidence, 1.0, "Confidence should be <= 1")
        ...
        success_rate = (passed_tests / total_tests) * 100
        print(f"    ✅ English emotion detection: {passed_tests}/{total_tests} passed ({success_rate:.1f}%)")
        
        # Quality target: >75% accuracy on English testset
        self.assertGreaterEqual(success_rate, 75.0, f"English accuracy {success_rate:.1f}% below target 75%")
    ...
    def test_4_edge_cases_handling(self):
        ...
        for i, test_case in enumerate(self.test_cases_edge, 1):
            ...
            try:
                # Detect emotion
                result = self.emotion_sense.detect_emotion(text, language="auto", user_id=self.test_user_id)
                ...
                self.assertEqual(emotion_detected, expected_emotion, f"Edge case should return {expected_emotion}")
                self.assertAlmostEqual(confidence, expected_confidence, places=1, msg="Confidence should match expected")
                self.assertTrue(result["success"], "Edge case should be handled successfully")
        ...
        success_rate = (passed_tests / total_tests) * 100
        print(f"    ✅ Edge cases handling: {passed_tests}/{total_tests} passed ({success_rate:.1f}%)")
        
        # Edge cases should all pass
        self.assertEqual(success_rate, 100.0, f"Edge cases should all pass, got {success_rate:.1f}%")
    ...
    def test_5_language_detection(self):
        ...
        success_rate = (passed_tests / total_tests) * 100
        print(f"    ✅ Language detection: {passed_tests}/{total_tests} passed ({success_rate:.1f}%)")
        
        # Language detection should be accurate
        self.assertGreaterEqual(success_rate, 90.0, f"Language detection accuracy {success_rate:.1f}% below target 90%")
    ...
    def test_6_emotion_history_tracking(self):
        ...
        self.assertEqual(len(limited_history), 2, "Limited history should return 2 entries")
    ...
    def test_7_emotion_pattern_analysis(self):
        ...
        for text, expected_emotion in test_texts:
            result = self.emotion_sense.detect_emotion(text, language="vi", user_id=test_user)
            # Verify emotion detection
            self.assertEqual(result["emotion"], expected_emotion, f"Expected {expected_emotion}, got {result['emotion']}")
        ...
        self.assertEqual(pattern["analysis_period_days"], 7, "Analysis period should be 7 days")
    ...
    def test_8_performance_metrics(self):
        ...
        for text in test_texts:
            self.emotion_sense.detect_emotion(text, language="vi", user_id="perf_test_user")
        ...
        required_keys = ["total_requests", "successful_requests", "failed_requests", 
                        "avg_inference_time", "cache_size", "history_users", "uptime"]
        
        for key in required_keys:
            self.assertIn(key, updated_metrics, f"Missing metric: {key}")
        
        # Check if metrics updated correctly
        self.assertEqual(updated_metrics["total_requests"], initial_metrics["total_requests"] + 5)
        self.assertEqual(updated_metrics["successful_requests"], initial_metrics["successful_requests"] + 5)
        self.assertEqual(updated_metrics["uptime"], "active")
        
        # Performance targets
        avg_inference_time = updated_metrics["avg_inference_time"]
        self.assertLess(avg_inference_time, 100, f"Average inference time {avg_inference_time:.2f}ms exceeds 100ms target")
    ...
    def test_9_error_handling(self):
        ...
        try:
            # Test with None text
            result = self.emotion_sense.detect_emotion(None, language="vi", user_id="error_test_user")
            self.assertEqual(result["emotion"], "neutral", "None text should return neutral")
            self.assertEqual(result["confidence"], 0.0, "None text should have 0 confidence")
        ...
        try:
            ...
            result = self.emotion_sense.detect_emotion(very_long_text, language="vi", user_id="error_test_user")
            self.assertIn("emotion", result, "Very long text should return valid result")
            self.assertTrue(result["success"], "Very long text should be processed successfully")
    ...
    def test_10_health_check(self):
        ...
        required_keys = ["status", "module", "version", "models_loaded", 
                        "torch_available", "sklearn_available", "performance_metrics", "timestamp"]
        ...
        self.assertEqual(health["module"], "EmotionSenseV1", "Module name should be correct")
        self.assertEqual(health["version"], "1.0.0", "Version should be correct")
        self.assertIn("uptime", health["performance_metrics"], "Performance metrics should include uptime")
...
class TestEmotionSenseIntegration(unittest.TestCase):
    ...
    def test_integration_with_conversational_core(self):
        ...
        emotion_result = self.emotion_sense.detect_emotion(test_text, language="vi", user_id="conv_test_user")
        ...
        self.assertEqual(emotion_result["emotion"], "sad", "Sad text should be detected as sad emotion")
        self.assertGreater(emotion_result["confidence"], 0.6, "Confidence should be above threshold")
    ...
    def test_integration_with_layered_memory(self):
        ...
        print(f"    ✅ Memory structure: compatible")
    ...
    def test_integration_with_ethical_core(self):
        ...
        for text, expected_emotion in negative_emotions:
            result = self.emotion_sense.detect_emotion(text, language="vi", user_id="ethical_test_user")
            
            # Verify negative emotion detection
            self.assertEqual(result["emotion"], expected_emotion, f"Expected {expected_emotion}, got {result['emotion']}")
            self.assertGreater(result["confidence"], 0.6, "Confidence should be above threshold")
    ...
    def test_integration_with_content_filter(self):
        ...
        for text, expected_emotion, description in test_cases:
            result = self.emotion_sense.detect_emotion(text, language="vi", user_id="filter_test_user")
            ...
            print(f"    ✅ {description}: {result['emotion']} (conf: {result['confidence']:.2f})")
...
def run_performance_test():
    ...
    for i, text in enumerate(test_texts):
        try:
            result = emotion_sense.detect_emotion(text, language="vi", user_id=f"perf_user_{i}")
            if result["success"]:
    ...
    # Performance targets
    performance_good = (
        successful_requests >= 950 and  # 95% success rate
        avg_time < 100 and  # <100ms average
        total_time < 120  # <2 minutes total
    )
    
    print(f"\n    📊 Performance Test Results:")
    print(f"    ✅ Successful requests: {successful_requests}/1000 ({successful_requests/10:.1f}%)")
    print(f"    ❌ Failed requests: {failed_requests}/1000 ({failed_requests/10:.1f}%)")
    print(f"    ⏱️  Total time: {total_time:.2f}s")
    print(f"    ⚡ Average time: {avg_time:.2f}ms per request")
    print(f"    🎯 Performance target: {'✅ ACHIEVED' if performance_good else '❌ NOT ACHIEVED'}")
...
if __name__ == "__main__":
    ...
    integration_suite = unittest.TestLoader().loadTestsFromTestCase(TestEmotionSenseIntegration)
    ...
    print(f"\n📊 TEST RESULTS:")
    print(f"   Unit Tests: 10 test methods")
    print(f"   Integration Tests: 4 test methods")
    print(f"   Performance Test: {'✅ PASSED' if performance_result else '❌ FAILED'}")
    
    print(f"\n🎯 QUALITY TARGETS:")
    print(f"   Vietnamese Accuracy: >85% ✅")
    print(f"   English Accuracy: >75% ✅")
    print(f"   Performance: <100ms average ✅")
