    ...
    def test_architecture_intent_detection_vi(self):
        ...
        for message, expected in test_cases:
            with self.subTest(message=message):
                is_architecture, locale = self.handler.detect_architecture_intent(message)
    ...
    def test_architecture_response_generation_vi(self):
        ...
        for message in test_messages:
            with self.subTest(message=message):
                ...
                self.assertTrue(
                    any(word in response for word in ["bí mật", "giữ kín", "riêng mình", "nghề nghiệp", "thể hiện", "hành động"])
                )
    ...
    def test_architecture_response_generation_en(self):
        ...
        for message in test_messages:
            with self.subTest(message=message):
                ...
                # Should contain security-friendly language
                self.assertTrue(
                    any(word in response for word in ["secret", "private", "myself", "professional", "demonstrate", "actions"])
                )
    ...
    def test_secure_response_priority(self):
        ...
        for question in architecture_questions:
            with self.subTest(question=question):
                ...
                # Should contain security-friendly language
                self.assertTrue(
                    any(word in response.lower() for word in ["secret", "private", "bí mật", "giữ kín", "myself", "riêng mình"])
                )
