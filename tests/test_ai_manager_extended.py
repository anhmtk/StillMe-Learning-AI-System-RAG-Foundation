...
from stillme_core.ai_manager import (
    set_mode, warmup, health, dev_agent, controller,
)
...
def test_fast_text_only_sanity():
    set_mode("fast"); warmup()
    # ràng buộc câu trả lời cực ngắn, dễ pass
    out = dev_agent("Reply with 'ok' only (lowercase). No punctuation, no spaces.", mode="fast")
    assert "ok" == out.strip().lower()

def test_code_mode_python_exec_digits_only():
    set_mode("code"); warmup()
