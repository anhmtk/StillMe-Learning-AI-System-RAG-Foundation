...
# Configuration
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "http://127.0.0.1:11434")  # Use 127.0.0.1 instead of localhost
...
def call_ollama_simple_stream(model: str, prompt: str, options: dict = None) -> Generator[Dict[str, Any], None, None]:
        ...
        for raw in resp.iter_lines(decode_unicode=True):
            if not raw:
                ...
                if not yielded and (time.time() - t0) > OLLAMA_INIT_CHUNK_TIMEOUT:
                    raise TimeoutError("No first chunk from Ollama within init-chunk timeout")
                continue
            yielded = True
            try:
                data = json.loads(raw)
...
def call_ollama_chat(model: str, messages: List[Dict[str, str]], stream: bool = True, options: dict = None, timeout: float = None) -> Generator[Dict[str, Any], None, None]:
    """
            ...
            with client.stream(
                ...
                for line in resp.iter_lines():
                    ...
                    if not hasattr(call_ollama_chat, '_first_chunk_time'):
                        ...
                        logger.info(f"First chunk in {call_ollama_chat._first_chunk_time:.2f}s")
...
def is_loaded(model: str) -> bool:
    ...
    try:
        ...
        r.raise_for_status()
        data = r.json()
        return any(p.get("model", "").startswith(model.split(":")[0]) for p in data.get("models", []))
    except Exception as e:
        logger.warning(f"Failed to check model status: {e}")
        return False

def generate(model: str, prompt: str, options=None, keep_alive="1h", stream=True) -> str:
    """Generate response with optimized defaults."""
    payload = {"model": model, "prompt": prompt, "stream": stream, "keep_alive": keep_alive}
    if options:
...
def _log_telemetry(model: str, prompt: str, start_time: float, stream: bool, ollama_meta: dict):
    """Log telemetry data"""
    try:
        ...
        if ollama_meta.get("total_duration"):
            generate_ms = int(ollama_meta["total_duration"] / 1000000)  # Convert from nanoseconds to ms
...
def warmup_blocking(model="gemma2:2b"):
    ...
    try:
        ...
        _ = generate(model, "hi", options={"num_predict": 8, "temperature": 0.1}, keep_alive="1h", stream=False)
...
def warmup_phi4_mini():
    ...
    try:
        ...
        _ = generate(model, "Hello, how are you?", 
                    options={"num_predict": 8, "temperature": 0.1}, 
                    keep_alive="1h", stream=False)
        
        # Wait for model to be confirmed loaded
        t0 = time.time()
        while not is_loaded(model) and time.time() - t0 < 15:  # Longer timeout for phi4-mini
            time.sleep(0.5)
