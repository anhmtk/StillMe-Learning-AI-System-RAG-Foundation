...
class MetricsCollector:
    ...
    def get_metrics(self) -> str:
        ...
        with self.lock:
            ...
            lines.append(
                f"requests_duration_seconds {self.metrics['requests_duration_seconds']:.3f}"
            )
            ...
            lines.append(
                f'requests_duration_bucket{{le="+Inf"}} {self.metrics["requests_total"]}'
            )
            ...
            lines.append(
                f"clarify_trigger_total {self.metrics['clarify_trigger_total']}"
            )
            ...
            lines.append(
                f"system_uptime_seconds {self.metrics['system_uptime_seconds']:.3f}"
            )
            ...
            lines.append(
                f"system_memory_usage_bytes {self.metrics['system_memory_usage_bytes']}"
            )

            lines.append("# HELP system_cpu_usage_percent System CPU usage percentage")
            lines.append("# TYPE system_cpu_usage_percent gauge")
            lines.append(
                f"system_cpu_usage_percent {self.metrics['system_cpu_usage_percent']:.3f}"
            )
