...
class DataValidator:
    ...
    def validate_string(value: Any, min_length: Optional[int] = None, 
                       max_length: Optional[int] = None, pattern: Optional[str] = None) -> str:
        """
    ...
    @staticmethod
    def validate_datetime(value: Union[str, datetime], format: str = "%Y-%m-%d %H:%M:%S") -> datetime:
        """
        Validate datetime
        Xác thực datetime
            ...
            for rule in rules:
                ...
                try:
                    ...
                    if rule.allowed_values and validated_value not in rule.allowed_values:
                        error_msg = f"Field '{field_name}' has invalid value: {validated_value}"
                        if rule.severity == ValidationSeverity.CRITICAL:
                            errors.append(error_msg)
                        else:
                            warnings.append(error_msg)
...
def validate_user_input(data: Dict[str, Any], 
                       ...
                       email_fields: Optional[List[str]] = None) -> ValidationResult:
    """
