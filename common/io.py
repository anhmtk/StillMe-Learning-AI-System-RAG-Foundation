...
from typing import Dict, Any, List, Optional, Union, BinaryIO, TextIO, Iterator
...
from .errors import ValidationError, StillMeException
...
class FileManager:
    ...
    def backup_file(self, path: Union[str, Path], backup_suffix: str = ".backup") -> Path:
        """
    ...
    def safe_write(self, path: Union[str, Path], data: Any, operation: FileOperation) -> None:
        """
        ...
        if path.exists() and not operation.overwrite:
            raise StillMeException(f"File already exists and overwrite is not allowed: {path}")
        ...
        try:
            ...
            logger.info("File written successfully", path=str(path), size=len(str(data)))
            
        except Exception as e:
            # Clean up temporary file
            if temp_path.exists():
                temp_path.unlink()
            raise StillMeException(f"Failed to write file {path}: {str(e)}") from e
    ...
    def read_file(self, path: Union[str, Path], format_type: Optional[FileFormat] = None) -> Any:
        """
        ...
        try:
            ...
            raise StillMeException(f"Failed to read file {path}: {str(e)}") from e
    ...
    async def async_read_file(self, path: Union[str, Path], format_type: Optional[FileFormat] = None) -> Any:
        """
    ...
    async def async_write_file(self, path: Union[str, Path], data: Any, operation: FileOperation) -> None:
        """
    ...
    async def _async_write_data(self, path: Path, data: Any, format_type: FileFormat) -> None:
        """Async write data to file - Ghi data vào file bất đồng bộ"""
        if format_type == FileFormat.JSON:
    ...
    def copy_file(self, source: Union[str, Path], destination: Union[str, Path], 
        ...
        if destination.exists() and not operation.overwrite:
            raise StillMeException(f"Destination file already exists and overwrite is not allowed: {destination}")
        ...
        try:
            ...
            raise StillMeException(f"Failed to copy file from {source} to {destination}: {str(e)}") from e
    ...
    def move_file(self, source: Union[str, Path], destination: Union[str, Path], 
        ...
        try:
            ...
            raise StillMeException(f"Failed to move file from {source} to {destination}: {str(e)}") from e
    ...
    def delete_file(self, path: Union[str, Path], backup: bool = False) -> None:
        ...
        try:
            ...
            raise StillMeException(f"Failed to delete file {path}: {str(e)}") from e
...
def write_json(path: Union[str, Path], data: Dict[str, Any], 
               backup: bool = False, overwrite: bool = True) -> None:
    """
...
def write_yaml(path: Union[str, Path], data: Dict[str, Any], 
               backup: bool = False, overwrite: bool = True) -> None:
    """
...
async def async_write_json(path: Union[str, Path], data: Dict[str, Any], 
                          backup: bool = False, overwrite: bool = True) -> None:
    """
