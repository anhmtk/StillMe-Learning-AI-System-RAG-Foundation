...
class StructuredLogger:
    ...
    def __init__(self, name: str, log_file: Optional[str] = None, 
                 level: int = logging.INFO, json_format: bool = True,
                 console_output: bool = True, max_file_size: int = 10*1024*1024,
                 backup_count: int = 5):
        """
    ...
    def log_api_call(self, method: str, url: str, status_code: int, 
        ...
        self.info(f"API Call: {method} {url} - {status_code} ({duration:.3f}s)",
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = time.time() - start_time
                logger.error(f"Performance: {op_name} failed after {duration:.3f}s", 
                           error=str(e), operation=op_name, duration=duration)
                raise
