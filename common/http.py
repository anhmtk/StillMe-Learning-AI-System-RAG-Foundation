...
from .errors import APIError, NetworkError, ValidationError, StillMeException
...
class AsyncHttpClient:
    ...
    def _prepare_data(self, request: HTTPRequest) -> Optional[Union[Dict[str, Any], str, bytes]]:
    ...
    async def _make_request(self, request: HTTPRequest) -> HTTPResponse:
        ...
        try:
            ...
            logger.error("HTTP request timeout",
                        ...
                        timeout=timeout,
                        elapsed_time=elapsed_time)
            raise NetworkError(f"Request timeout after {timeout}s", url=full_url, timeout=timeout) from e
            ...
            logger.error("HTTP request error",
                        ...
                        error=str(e),
                        elapsed_time=elapsed_time)
            raise NetworkError(f"Request failed: {str(e)}", url=full_url) from e
            ...
            logger.error("Unexpected HTTP error",
                        ...
                        error=str(e),
                        elapsed_time=elapsed_time)
            raise StillMeException(f"Unexpected HTTP error: {str(e)}") from e
...
class ResponseValidator:
    ...
    @staticmethod
    def validate_json_response(response: HTTPResponse, required_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if required_fields:
            missing_fields = [field for field in required_fields if field not in response.json_data]
            if missing_fields:
