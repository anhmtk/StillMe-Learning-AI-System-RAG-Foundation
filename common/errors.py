    ...
    def handle_error(self, error: Exception, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if self.logger:
            if isinstance(error, StillMeException):
                self.logger.error(f"StillMe Error: {error.message}", 
...
class ErrorRecovery:
    ...
    def retry_with_backoff(func, max_retries: int = 3, 
                          base_delay: float = 1.0, exceptions: tuple = (Exception,)):
        """
    ...
    @staticmethod
    def fallback_response(error: StillMeException, 
...
def handle_error(error: Exception, logger=None, context: Dict[str, Any] = None) -> Dict[str, Any]:
    """Handle error with default error handler"""
    handler = ErrorHandler(logger)
    return handler.handle_error(error, context)


def create_error_response(error: Exception, fallback_data: Any = None) -> Dict[str, Any]:
