...
CODE_MARKERS = [
    "```", "def ", "class ", "#include", "<script", "SELECT ", "INSERT ", "UPDATE ", "DELETE ",
    "for(", "while(", "try:", "except", "pip install", "npm install", "curl "
]
CREATIVE_HINTS = ["viết thơ", "làm thơ", "poem", "story", "kể chuyện", "sáng tác"]
CHITCHAT_HINTS = ["hello", "xin chào", "chào", "hi ", "hey", "how are you", "dạo này sao"]
CODING_HINTS = ["python", "java", "c++", "bug", "lỗi", "error", "debug", "thuật toán", "sql", "api"]
SAFETY_HINTS = {
    "cyber": ["hack", "ddos", "sql injection", "xss", "malware", "keylogger", "phá khóa"],
    "pii": ["cccd", "số căn cước", "địa chỉ nhà", "số điện thoại riêng", "gmail của người này"],
    "weapon": ["bom xăng", "thuốc nổ", "explosive", "bomb"],
    "hate": ["xúc phạm nhóm", "insult group", "hate"],
    "medical": ["đau ngực", "triệu chứng", "paracetamol", "thuốc", "bị sốt", "có nguy hiểm"],
...
def guess_lang(text: str) -> str:
    t = text.lower()
    return "vi" if any(ch in t for ch in VI_CHARS) else "en" if re.fullmatch(r"[ -~]+", t or "") else "mix"
