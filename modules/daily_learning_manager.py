...
class DailyLearningManager:
    ...
    def __init__(self, cases_file: str = "daily_learning_cases.json", memory_manager=None, improvement_manager=None):
        ...
        if self.memory_manager:
            ...
            self.logger.warning("⚠️ No memory manager provided - learning results won't be stored in memory")
        
        # Integration với SelfImprovementManager
        self.improvement_manager = improvement_manager
        if self.improvement_manager:
            self.logger.info("✅ DailyLearningManager integrated with SelfImprovementManager")
        else:
            self.logger.warning("⚠️ No improvement manager provided - no automatic improvement suggestions")
    ...
    def add_new_case(self, category: str, question: str, expected_keywords: List[str], 
                    difficulty: str = "medium", language: str = "vi") -> bool:
    ...
    def record_response(self, case_id: str, response: str, score: float, feedback: str = "") -> bool:
    ...
    def _save_to_memory(self, case_id: str, response: str, score: float, feedback: str):
        ...
        try:
            ...
            memory_item = LearningMemoryItem(
                ...
                keywords_matched=self._analyze_keywords_matched(case.expected_keywords, response),
                learning_insights=self._extract_learning_insights(response, score),
                improvement_suggestions=self._generate_improvement_suggestions(case, score, feedback)
            )
            ...
            self.logger.info(f"✅ Learning result saved to memory: {case_id} (score: {score:.2f})")
            
        except Exception as e:
            self.logger.error(f"Error saving to memory: {e}")
    
    def _analyze_keywords_matched(self, expected_keywords: List[str], response: str) -> List[str]:
    ...
    def _generate_improvement_suggestions(self, case: LearningCase, score: float, feedback: str) -> List[str]:
        ...
        if len(case.expected_keywords) > len(self._analyze_keywords_matched(case.expected_keywords, case.response)):
...
Feedback: {memory_item.feedback}
    ...
    def _log_interaction(self, case_id: str, response: str, score: float, feedback: str):
    ...
    def search_learning_memory(self, query: str, category: Optional[str] = None, min_score: Optional[float] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.memory_manager:
                try:
                    ...
                    all_learning_results = self.search_learning_memory("learning_result")
                    memory_stats = {
                        "total_learning_results": len(all_learning_results),
                        "avg_memory_score": sum(r["score"] for r in all_learning_results) / len(all_learning_results) if all_learning_results else 0,
                        "categories_in_memory": list(set(r["category"] for r in all_learning_results))
                    }
                except Exception as e:
...
"""
        ...
        try:
            ...
            if self.memory_manager:
                try:
                    ...
                    recent_results = sorted(all_results, key=lambda x: x.get('timestamp', ''), reverse=True)[:10]
                except Exception as e:
                    self.logger.warning(f"Could not get recent results: {e}")
            
            # Phân tích performance patterns
            performance_analysis = self._analyze_performance_patterns(recent_results, stats)
            
            # Tạo improvement suggestions
            improvement_suggestions = self._generate_system_improvements(performance_analysis)
    ...
    def _analyze_performance_patterns(self, recent_results: List[Dict], stats: Dict) -> Dict[str, Any]:
        ...
        if analysis["weak_categories"]:
            analysis["improvement_areas"].append(f"Focus on categories: {', '.join(analysis['weak_categories'])}")
        
        if analysis["overall_score_trend"] == "declining":
            analysis["improvement_areas"].append("Overall performance declining - need review")
        
        return analysis
    
    def _generate_system_improvements(self, performance_analysis: Dict) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                change_data = {
                    ...
                    "risk_level": "LOW" if suggestion["priority"] == "low" else "MEDIUM",
