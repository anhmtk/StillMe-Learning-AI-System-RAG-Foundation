...
class SemanticHybridCache:
    ...
    def get_semantic_match(self, query: str, threshold: float) -> Optional[Tuple[CacheItem, float]]:
...
class TokenOptimizer:
    ...
    def process_request(self, query: str, context: List[str], api_callback: callable) -> Tuple[str, bool]:
        ...
        try:
            ...
            semantic_match = self.cache.get_semantic_match(query, self.config.min_similarity_threshold)
            if semantic_match:
                item, similarity = semantic_match
                logger.info(f"Semantic cache hit (similarity: {similarity:.2f}) for query: {query[:50]}...")
            ...
            self.quality_monitor.log_quality(original_query=query, optimized_response=response)
            return response, False

        except Exception as e:
            logger.error(f"Optimization failed, using fallback: {str(e)}")
    ...
    def _optimize_prompt(self, query: str, context: List[str]) -> str:
        ...
        if (self._count_tokens(optimized) > self.config.max_prompt_tokens and
