...
try:
    from modules.content_integrity_filter import ContentIntegrityFilter as _ContentIntegrityFilter
...
class SelfImprovementManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "critical_files": [
                "framework.py", "app.py", "modules/ethical_core_system.py",
    ...
    def _generate_change_id(self, change_type: str, file_path: str) -> str:
        ...
        content_hash = hashlib.md5(f"{change_type}_{file_path}_{timestamp}".encode()).hexdigest()[:8]
    ...
    def _is_file_allowed(self, file_path: str) -> bool:
        ...
        if str(path) in self.config["critical_files"]:
            self.logger.warning(f"Critical file modification requires special approval: {file_path}")
        ...
        try:
            ...
            for change in proposed_changes:
                if self._safety_check_change(change):
                    ...
                    self.logger.warning(f"Change rejected by safety check: {change['id']}")
    ...
    def _read_daily_learning_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_cases_available": learning_manager.cases_data.get("metadata", {}).get("total_cases", 0)
            }
    ...
    def _read_system_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "test_count": len(list(Path("tests").glob("test_*.py"))) if Path("tests").exists() else 0,
    ...
    def _analyze_with_ai(self, analysis_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if analysis_data["error_logs"]:
            ...
            proposed_changes.append({
                "id": self._generate_change_id("code", "modules/api_provider_manager.py"),
                "timestamp": datetime.now().isoformat(),
                "change_type": "code",
        ...
        self.logger.info(f"AI analysis generated {len(proposed_changes)} proposed changes")
    ...
    def _safety_check_change(self, change: Dict[str, Any]) -> bool:
        ...
        if not self._is_file_allowed(change["file_path"]):
            self.logger.warning(f"File not allowed for modification: {change['file_path']}")
            return False
        ...
        if change["risk_level"] in ["HIGH", "CRITICAL"]:
            self.logger.warning(f"High risk change requires manual approval: {change_id}")
    ...
    def _save_proposed_changes(self, changes: List[Dict[str, Any]]) -> None:
        ...
        self.logger.info(f"Saved {len(proposed_changes)} proposed changes to {self.proposed_changes_file}")
    ...
    def get_proposed_changes(self) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(self.proposed_changes_file):
            try:
                with open(self.proposed_changes_file, 'r', encoding='utf-8') as f:
                    return json.load(f)
            except Exception as e:
    ...
    def approve_change(self, change_id: str, approved: bool = True) -> bool:
        ...
        for change in changes:
            if change["id"] == change_id:
                ...
                self.logger.info(f"Change {change_id} {'approved' if approved else 'rejected'}")
                return True
                    ...
                    if self._test_in_sandbox(change):
                        ...
                        if self._apply_change_real(change):
                            ...
                            if self._run_full_test_suite():
                                results["applied"] += 1
                                change["applied"] = True
                                self.logger.info(f"Successfully applied change: {change['id']}")
                            else:
                                # Rollback nếu test fail
                                self._rollback_change(change, backup_path)
                                results["rollbacks"] += 1
                                results["failed"] += 1
                                self.logger.error(f"Rolled back change due to test failure: {change['id']}")
                        else:
                            results["failed"] += 1
                            self.logger.error(f"Failed to apply change: {change['id']}")
                    else:
                        results["failed"] += 1
                        self.logger.error(f"Sandbox test failed for change: {change['id']}")
                else:
                    results["failed"] += 1
                    self.logger.error(f"Failed to apply change in sandbox: {change['id']}")
                
                results["details"].append({
                    "change_id": change["id"],
                    "status": "applied" if change.get("applied", False) else "failed"
                })
    ...
    def get_safety_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "pending": len([c for c in changes if not c.get("approved", False) and not c.get("applied", False)]),
            "safety_mode": self.safety_mode,
            "read_only_mode": self.read_only_mode,
            "risk_distribution": {
                ...
                "CRITICAL": len([c for c in changes if c.get("risk_level") == "CRITICAL"])
            },
            "safety_checks_summary": {
                "ethical_passed": len([c for c in changes if c.get("safety_checks", {}).get("ethical", False)]),
                "integrity_passed": len([c for c in changes if c.get("safety_checks", {}).get("integrity", False)]),
                "overall_safe": len([c for c in changes if c.get("safety_checks", {}).get("overall", False)])
            }
        }
    ...
    def emergency_rollback_all(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if backup_files:
                    ...
                    if self._rollback_change(change, str(latest_backup)):
                        ...
                        self.logger.info(f"Emergency rollback successful: {change['id']}")
                    else:
                        results["failed_rollbacks"] += 1
                        self.logger.error(f"Emergency rollback failed: {change['id']}")
                else:
                    results["failed_rollbacks"] += 1
                    self.logger.error(f"No backup found for emergency rollback: {change['id']}")
                
                results["details"].append({
                    "change_id": change["id"],
                    "rollback_status": "success" if results["rollback_count"] > 0 else "failed"
                })
        ...
        self.logger.warning(f"Emergency rollback completed: {results['rollback_count']} successful, {results['failed_rollbacks']} failed")
        return results


# Factory function để tạo instance
def create_self_improvement_manager(config_path: str = "config/self_improvement_config.json") -> SelfImprovementManager:
    """
