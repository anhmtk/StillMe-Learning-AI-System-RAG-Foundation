...
class OpenRouterClient:
    ...
    async def chat_completion(self, model: OpenRouterModel, messages: list, temperature: float = 0.7, max_tokens: int = 500) -> str:
        ...
        try:
            ...
            logging.error(f"Lỗi HTTP khi gọi OpenRouter API: {e.response.status_code} - {e.response.text}")
...
class PersonaManager:
    ...
    def __init__(self, user_profiles_db_path=USER_PROFILES_DB_PATH):
        ...
        self._load_profiles()
        logging.info(f"PersonaManager: Khởi tạo với DB hồ sơ người dùng tại {user_profiles_db_path}")
    ...
    def _load_profiles(self):
        ...
        if os.path.exists(self.user_profiles_db_path):
            with open(self.user_profiles_db_path, "r", encoding='utf-8') as f:
                try:
                    profiles = json.load(f)
                    for user_id, profile_data in profiles.items():
                        self.user_profiles[user_id] = {
                            "style_history": [StyleFeatures.from_dict(f) for f in profile_data.get("style_history", [])],
                            "current_style": StyleFeatures.from_dict(profile_data.get("current_style", {}))
                        }
                except json.JSONDecodeError:
                    logging.warning(f"Lỗi đọc file JSON {self.user_profiles_db_path}. Tạo lại file trống.")
                    self.user_profiles = defaultdict(dict)
        else:
            logging.info(f"File hồ sơ người dùng {self.user_profiles_db_path} không tồn tại, sẽ tạo mới.")
    ...
    def _save_profiles(self):
        ...
        for user_id, profile in self.user_profiles.items():
            profiles_to_save[user_id] = {
                "style_history": [f.to_dict() for f in profile.get("style_history", [])],
                "current_style": profile.get("current_style", StyleFeatures()).to_dict()
            }
        try:
            ...
            logging.error(f"Không thể lưu hồ sơ người dùng vào {self.user_profiles_db_path}: {e}")
...
class PersonaGenerator:
    ...
    def __init__(self, openrouter_client: OpenRouterClient):
        self.client = openrouter_client
        self.generation_model = OpenRouterModel.GPT_3_5_TURBO # Chọn một mô hình cho việc sinh văn bản
        logging.info(f"PersonaGenerator: Khởi tạo với mô hình sinh văn bản {self.generation_model.value}.")

    async def generate_styled_response(self, original_response: str, ai_persona: StyleFeatures) -> str:
        """
...
class PersonaMorph:
    ...
    async def process_interaction(self, user_id: str, user_text: str, original_ai_response: str) -> str:
        """
        ...
        styled_response = await self.generator.generate_styled_response(original_ai_response, ai_persona)
