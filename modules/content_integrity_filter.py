...
file_handler = logging.FileHandler(os.path.join(LOGS_DIR, 'content_filter.log'), encoding='utf-8')
...
class OpenRouterClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://openrouter.ai/api/v1/chat/completions"):
        ...
        }, timeout=90.0) # Tăng timeout cho các tác vụ phân tích nặng và đường truyền kém
    ...
    async def chat_completion(self, model: OpenRouterModel, messages: list, temperature: float = 0.1, max_tokens: int = 700, response_format: Dict = None) -> str:
        ...
        try:
            ...
            logger.error(f"OpenRouter API Response Parse Error: {e} - Raw: {response.text if 'response' in locals() else 'No response'}")
            raise
...
class ContentIntegrityFilter:
    ...
    def _load_rules(self) -> Dict:
        ...
        default_rules = {
            ...
            "unreliable_sources": [
                "somehoaxsite.com", "fake-news-daily.net", "conspiracytheorycentral.org",
                "trollfarm.ru", "extremist-blogs.info", "hoax-buster.net"
            ],
        ...
        try:
            if os.path.exists(CONTENT_RULES_PATH):
                ...
                logger.warning(f"File rules không tồn tại tại {CONTENT_RULES_PATH}. Đang tạo file mặc định.")
        except json.JSONDecodeError:
            logger.error(f"Lỗi đọc JSON từ {CONTENT_RULES_PATH}. Đang sử dụng rules mặc định.")
        except Exception as e:
    ...
    def _normalize_text(self, text: str) -> str:
        ...
        for char, variants in self.normalization_map.items():
            for variant in variants:
                if variant in text: # Kiểm tra tồn tại để tránh thay thế không cần thiết
    ...
    async def pre_filter_content(self, content_text: str, source_url: Optional[str] = None) -> Tuple[bool, str, Severity]:
        """
        ...
        if not content_text or len(content_text) < min_len:
            return False, f"Nội dung quá ngắn ({len(content_text)} ký tự), không đủ chất lượng.", Severity.LOW
        ...
        if source_url:
            ...
            for unreliable_domain in self.rules.get("unreliable_sources", []):
                ...
                if normalized_source_url == normalized_unreliable_domain:
                    return False, f"Nguồn '{source_url}' được xác định là không đáng tin cậy.", Severity.HIGH
    ...
    async def analyze_content_quality(self, content_text: str) -> Dict[str, Any]:
        ...
        default_analysis = {
            "toxicity_score": 0.0, "hate_speech_score": 0.0, "bias_score": 0.0,
        ...
        try:
            llm_response_str = await self.openrouter_client.chat_completion(
                self.quality_analysis_model, messages, temperature=0.1, max_tokens=700, response_format=response_format
            )
            ...
            validated_analysis.update({
                "toxicity_score": max(0.0, min(1.0, float(analysis_data.get("toxicity_score", 0.0)))),
                "hate_speech_score": max(0.0, min(1.0, float(analysis_data.get("hate_speech_score", 0.0)))),
                "bias_score": max(0.0, min(1.0, float(analysis_data.get("bias_score", 0.0)))),
                "biased_aspects": analysis_data.get("biased_aspects", []),
                "sensitive_topics_detected": analysis_data.get("sensitive_topics_detected", []),
                "overall_assessment": analysis_data.get("overall_assessment", "Không phân loại được.")
            })
    ...
    async def fact_check_content(self, content_text: str) -> Tuple[bool, float, str]:
        ...
        """
        # Trả về mặc định an toàn nhưng báo lỗi nếu có vấn đề LLM
        default_fact_check = (False, 0.0, "Lỗi phân tích LLM hoặc không thể kiểm tra sự thật.")
        ...
        try:
            ...
            is_factual = bool(fact_check_data.get("is_factual", False)) # Mặc định là False nếu không rõ ràng
            confidence_score = max(0.0, min(1.0, float(fact_check_data.get("confidence_score", 0.0)))) # Clamp to 0-1
            ...
            logger.error(f"Lỗi khi kiểm chứng thông tin bằng LLM: {e}. Trả về mặc định an toàn (có báo lỗi).")
            return default_fact_check + ([],) # Bổ sung misinformation_detected trống


    def log_content_violation(self, content_id: str, content_text: str, source_url: Optional[str], 
                              violation_type: ContentViolationType, severity: Severity, details: str,
        ...
        """
        truncated_content_text = content_text[:500] + "..." if len(content_text) > 500 else content_text
        ...
        logger.info(f"Đã ghi log vi phạm nội dung cho ContentID {content_id}: {details} (Mức: {severity.value})")

    async def filter_content(self, content_id: str, content_text: str, source_url: Optional[str] = None) -> Tuple[bool, str, Severity, Dict[str, Any]]:
        """
        ...
        logger.info(f"Content Preview: \"{content_text[:100]}...\" | Source: {source_url}")
        ...
        pre_filter_safe, pre_filter_reason, pre_filter_severity = await self.pre_filter_content(content_text, source_url)
        if not pre_filter_safe:
            violation_type = ContentViolationType.FORBIDDEN_KEYWORD if "từ khóa cấm" in pre_filter_reason else \
                             ContentViolationType.UNRELIABLE_SOURCE if "nguồn không đáng tin cậy" in pre_filter_reason else \
                             ContentViolationType.SPAM_OR_LOW_QUALITY
            
            violation_reasons.append(f"Tiền lọc: {pre_filter_reason}")
            overall_severity = max(overall_severity, pre_filter_severity)
            self.log_content_violation(content_id, content_text, source_url, violation_type, pre_filter_severity, pre_filter_reason)
            ...
            if pre_filter_severity >= Severity.HIGH:
                ...
                logger.warning(f"Nội dung bị chặn sớm ở giai đoạn tiền lọc do mức độ nghiêm trọng: {final_reason}")
        ...
        if quality_analysis["toxicity_score"] >= toxicity_threshold:
            ...
            self.log_content_violation(content_id, content_text, source_url, ContentViolationType.TOXIC_CONTENT, Severity.HIGH, reason_detail)
        
        if quality_analysis["hate_speech_score"] >= hate_speech_threshold:
            is_safe = False
            overall_severity = max(overall_severity, Severity.CRITICAL)
            reason_detail = f"Lời nói thù ghét (score: {quality_analysis['hate_speech_score']:.2f})"
            violation_reasons.append(reason_detail)
            self.log_content_violation(content_id, content_text, source_url, ContentViolationType.HATE_SPEECH, Severity.CRITICAL, reason_detail)
        
        if quality_analysis["bias_score"] >= bias_threshold:
            is_safe = False
            biased_aspects = ", ".join(quality_analysis.get("biased_aspects", [])) or "không xác định"
            ...
            violation_reasons.append(reason_detail)
            self.log_content_violation(content_id, content_text, source_url, ContentViolationType.BIASED_CONTENT, Severity.MEDIUM, reason_detail)
        
        # 2b. Kiểm chứng Thông tin (Fact-Checking)
        # Bổ sung thêm `misinformation_detected` từ kết quả của LLM
        fact_check_safe, confidence_score, fact_check_reason, misinformation_detected = await self.fact_check_content(content_text)
        ...
        logger.info(f"--- Kết thúc lọc nội dung cho ContentID: {content_id}. Safe: {is_safe}, Severity: {overall_severity.value} ---")
        return is_safe, final_reason, overall_severity, detailed_analysis
