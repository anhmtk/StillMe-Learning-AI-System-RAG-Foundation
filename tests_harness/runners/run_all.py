...
def main():
    parser = argparse.ArgumentParser(
        description="Run comprehensive Test & Evaluation Harness"
    )
    parser.add_argument(
        "--since",
        type=int,
    ...
    parser.add_argument(
        "--offline", action="store_true", help="Run in offline mode with mock providers"
    )
    parser.add_argument(
        "--samples",
        type=int,
    ...
    try:
        ...
        if not args.skip_generation:
            ...
            dataset_file = (
                ...
                / f"large_dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            generate_large_dataset(
                max_samples=args.samples, output_file=str(dataset_file)
            )
        ...
        analyzer = OptimizationAnalyzer(
            reports_dir=args.output_dir, slo_policy_path="slo_policy.yaml"
        )
        ...
        summary = generate_summary_report(
            comprehensive_results, benchmark_results, analysis_results, args
        )
