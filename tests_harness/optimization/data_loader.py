...
class DataLoader:
    ...
    def _load_slo_policy(self) -> dict[str, Any]:
        ...
        try:
            if self.slo_policy_path.exists():
                ...
                logger.warning(
                    f"SLO policy not found: {self.slo_policy_path}, using defaults"
                )
        ...
        for json_file in self.reports_dir.glob("*.json"):
            ...
            try:
                with open(json_file, encoding="utf-8") as f:
                    data = json.load(f)
                    validated_data = self._validate_and_fill_defaults(
                        data, json_file.stem
                    )
    ...
    def _validate_and_fill_defaults(
        ...
        if "security" not in data:
            data["security"] = {"sandbox_egress_blocked": False, "attacks": {}}
        else:
