...
from ..core.message_protocol import MessageProtocol, MessageType, ResponseMessage
...
class StillMeIntegration:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if self.settings.STILLME_API_KEY:
                self.session.headers['Authorization'] = f'Bearer {self.settings.STILLME_API_KEY}'
        ...
        try:
            if not self.is_initialized or not self.session:
                return self._create_error_response(
                    message.id, 
                    "StillMe integration not initialized"
                )
    ...
    def _create_success_response(self, message_id: str, result: Dict[str, Any]) -> Dict[str, Any]:
