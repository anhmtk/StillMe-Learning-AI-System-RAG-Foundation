...
class NotificationService:
    ...
    async def get_notification_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending = len([n for n in self.notification_queue if n.get('status') == 'pending'])
            delivered = len([n for n in self.notification_queue if n.get('status') == 'delivered'])
            failed = len([n for n in self.notification_queue if n.get('status') == 'failed'])
