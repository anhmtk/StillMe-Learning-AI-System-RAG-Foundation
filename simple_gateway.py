...
async def send_to_ai_server(message: str, language: str = "vi") -> str:
    ...
    try:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(data, dict):
                            return data.get("text", data.get("response", data.get("message", data.get("content", str(data)))))
                        else:
                            return str(data)
        ...
        return f"Xin chào! Tôi là StillMe AI. Bạn đã nói: '{message}'. Tôi đang hoạt động qua Gateway! (Lỗi kết nối AI Server: {str(e)})"

@app.get("/", response_class=HTMLResponse)
async def root():
    """Root endpoint - Web Chat Interface"""
    try:
        with open("web_chat_interface.html", "r", encoding="utf-8") as f:
            html_content = f.read()
