...
def build_import_graph(root_packages: List[str]) -> Dict[str, Any]:
    ...
    return {"modules": all_modules, "inbound_counts": inbound_counts}
...
def save_import_data(data: Dict[str, Any], output_file: Path):
    ...
    for module, count in data["inbound_counts"].items():
        import_inbound.append({"module": module, "inbound_imports": count})
...
def main():
    ...
    if not root_packages:
        logger.error(
            "No root packages found. Expected: stillme_core, stillme_ethical_core"
        )
        return
    ...
    logger.info(
        f"  - Modules with inbound imports: {len([m for m, c in graph_data['inbound_counts'].items() if c > 0])}"
    )
