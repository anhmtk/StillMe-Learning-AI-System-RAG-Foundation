...
class SimpleImportOptimizer:
    ...
    def analyze_imports(self, file_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            return {
                ...
                'import_density': (len(imports) + len(from_imports)) / max(code_lines, 1) * 100
            }
    ...
    def generate_report(self, target_dirs: List[Path]) -> None:
        ...
        if high_density_files:
            ...
            for file_info in sorted(high_density_files, key=lambda x: x['density'], reverse=True):
                print(f"   * {file_info['file']} (density: {file_info['density']:.1f}%, imports: {file_info['imports']})")
        else:
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python tools/simple_import_optimizer.py <comma_separated_target_dirs> [root_path]")
        sys.exit(1)
