...
def generate_csv_report(ignores: List[Dict[str, str]], output_file: str = "artifacts/type_ignores.csv"):
    """Tạo CSV report với tất cả type ignores."""
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        if ignores:
            writer = csv.DictWriter(f, fieldnames=['file', 'line', 'code_context', 'full_line'])
            writer.writeheader()
...
def generate_summary_report(ignores: List[Dict[str, str]], output_file: str = "artifacts/type_ignores_summary.md"):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        if ignores:
            ...
            if len(ignores) > 50:
                f.write(f"... and {len(ignores) - 50} more (see CSV for complete list)\n")
        else:
...
def main():
    ...
    print(f"\n📄 Reports generated:")
    print(f"  - artifacts/type_ignores.csv")
    print(f"  - artifacts/type_ignores_summary.md")
