...
class ImpactAnalyzer:
    ...
    def analyze_impact(self, task: str) -> ImpactAnalysisResult:
        ...
        overall_risk = self._assess_overall_risk(
            security_risks, performance, user_impact
        )

        # Generate recommendations
        recommendations = self._generate_recommendations(
            dependencies, security_risks, performance
        )
    ...
    def _analyze_dependencies_enhanced(self, task: str) -> list[DependencyInfo]:
        ...
        if any(
            ...
            dependencies.extend(
                [
                    DependencyInfo(
                        "database", "database/connection.py", "import", 1, True, 1
                    ),
                    DependencyInfo(
                        "sqlalchemy", "database/models.py", "from_import", 2, True, 1
                    ),
                    DependencyInfo(
                        "pymongo", "database/mongo.py", "import", 3, True, 1
                    ),
                ]
            )
        ...
        if any(
            ...
            dependencies.extend(
                [
                    ...
                    DependencyInfo(
                        "cryptography", "security/encryption.py", "import", 3, True, 1
                    ),
                ]
            )
        ...
        if any(
            keyword in task_lower
            for keyword in ["performance", "optimize", "cache", "memory", "speed"]
        ):
            dependencies.extend(
                [
                    DependencyInfo("redis", "cache/manager.py", "import", 1, True, 1),
                    DependencyInfo(
                        "psutil", "monitoring/performance.py", "import", 2, True, 1
                    ),
                ]
            )
        ...
        if any(
            keyword in task_lower
            for keyword in ["test", "testing", "unit", "integration", "spec"]
        ):
            dependencies.extend(
                [
                    DependencyInfo("pytest", "tests/conftest.py", "import", 1, True, 1),
                    DependencyInfo(
                        "unittest", "tests/test_core.py", "import", 2, True, 1
                    ),
                ]
            )
        ...
        if any(
            keyword in task_lower
            for keyword in ["ui", "frontend", "web", "html", "css", "javascript"]
        ):
            dependencies.extend(
                [
                    DependencyInfo(
                        "flask", "templates/base.html", "template", 1, True, 1
                    ),
    ...
    def _analyze_performance(self, task: str) -> PerformanceImpact:
        ...
        if any(
            keyword in task_lower
            for keyword in ["optimize", "performance", "speed", "fast"]
        ):
    ...
    def _analyze_security_risks(self, task: str) -> list[SecurityRisk]:
        ...
        if any(
            keyword in task_lower
            for keyword in ["sql", "query", "database", "user input"]
        ):
            risks.append(
                SecurityRisk(
        ...
        if any(
            keyword in task_lower
            for keyword in ["auth", "login", "password", "authentication"]
        ):
            risks.append(
                SecurityRisk(
        ...
        if any(
            keyword in task_lower
            for keyword in ["html", "web", "frontend", "user input", "display"]
        ):
            risks.append(
                SecurityRisk(
    ...
    def _analyze_user_impact(self, task: str) -> UserImpact:
        ...
        if any(
            keyword in task_lower
            for keyword in ["user", "interface", "ui", "frontend", "login"]
        ):
            level = ImpactLevel.HIGH
    ...
    def _analyze_business_value(self, task: str) -> BusinessValue:
        ...
        if any(
            keyword in task_lower
            for keyword in ["security", "vulnerability", "fix", "critical"]
        ):
            roi_estimate = 0.9
            ...
            keyword in task_lower for keyword in ["optimize", "performance", "speed"]
        ):
            roi_estimate = 0.7
    ...
    def _assess_overall_risk(
        ...
        if security_risks and any(
            risk.severity == "CRITICAL" for risk in security_risks
        ):
            return RiskLevel.CRITICAL
        elif security_risks and any(risk.severity == "HIGH" for risk in security_risks):
            return RiskLevel.HIGH
        elif (
            performance.level == ImpactLevel.HIGH
            or user_impact.level == ImpactLevel.HIGH
        ):
