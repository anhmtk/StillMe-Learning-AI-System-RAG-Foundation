...
class StructuredLogger:
    ...
    def _load_config(self, config_path: Optional[str] = None) -> dict[str, Any]:
        ...
        if config_file.exists():
            ...
            return {
                ...
                "correlation": {"enabled": True, "auto_generate": True},
                "performance": {
                    "enabled": True,
    ...
    def _setup_standard_logging(self):
        ...
        if self.config["outputs"]["console"]["enabled"]:
            ...
            console_handler.setLevel(
                getattr(logging, self.config["outputs"]["console"]["level"])
            )
        ...
        if self.config["outputs"]["file"]["enabled"]:
            ...
            file_handler.setLevel(
                getattr(logging, self.config["outputs"]["file"]["level"])
            )
    ...
    def log_security(
        ...
        self._log(
            level, LogCategory.SECURITY, message, "agentdev.security", attributes, tags
        )
    ...
    def get_log_statistics(self) -> dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_logs = [
                log for log in self.log_entries if time.time() - log.timestamp < 3600
            ]  # Last hour
            error_count = len(
                [
                    log
...
def log_task_start(
    ...
    """Log task start"""
    attrs = attributes or {}
    attrs.update({"task_type": task_type, "start_time": time.time()})
...
def log_task_complete(
    ...
    """Log task completion"""
    attrs = attributes or {}
    attrs.update({"duration": duration, "end_time": time.time()})
...
def log_performance_metric(
    ...
    attrs = attributes or {}
    attrs.update({"metric_name": metric_name, "value": value, "unit": unit})
    logger.log_performance(
        LogLevel.INFO, f"Performance metric: {metric_name} = {value} {unit}", attrs
    )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        log.log_task(
            ...
            "Task started",
            "task_123",
            {"task_type": "deploy_edge", "target": "production"},
        )

        log.log_security(
            LogLevel.WARN,
            "Suspicious activity detected",
            {"event_type": "multiple_failed_logins", "ip_address": "192.168.1.100"},
        )

        log.log_performance(
            LogLevel.INFO,
            "High memory usage",
            {"memory_percent": 85.5, "threshold": 80.0},
        )
        ...
        log.log_audit(
            ...
            "User action",
            "user_789",
            {"action": "deploy_edge", "resource": "production"},
        )
