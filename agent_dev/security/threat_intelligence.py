...
class ThreatIntelligence:
    ...
    def add_indicator(self, indicator_type: ThreatType, value: str,
        ...
        if (self.config['blocking']['auto_block'] and
            confidence >= self.config['blocking']['block_threshold'] and
            value not in self.config['blocking']['whitelist']):
            self.blocked_indicators.add(indicator_id)
            print(f"🚫 Auto-blocked threat indicator: {value}")

        return indicator_id

    def check_indicator(self, indicator_type: ThreatType, value: str) -> Optional[ThreatIndicator]:
            ...
            if vt_data and vt_data.get('response_code') == 1:
                ...
                if confidence > 0.1:  # 10% detection threshold
                    analysis_result.update({
                        ...
                        'confidence': confidence,
                        'sources': ['virustotal'],
                        'recommendations': ['Block IP address', 'Monitor for additional activity']
                    })

            if abuse_data and abuse_data.get('data', {}).get('abuseConfidencePercentage', 0) > 25:
    ...
    def get_threat_statistics(self) -> Dict[str, Any]:
        ...
        for indicator in self.indicators.values():
            ...
            indicators_by_type[indicator_type] = indicators_by_type.get(indicator_type, 0) + 1
        ...
        return {
            ...
            'sources_active': len([s for s in self.intelligence_sources.values() if s.get('enabled', False)]),
            'last_update': max([s.get('last_update', 0) for s in self.intelligence_sources.values()])
        }
