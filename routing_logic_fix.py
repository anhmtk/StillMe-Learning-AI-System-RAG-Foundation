...
class SmartRouter:
    ...
    def call_gemma_local(self, payload: Dict, timeout: float = None) -> Dict:
        """Gọi Gemma local với timeout ngắn"""
        if timeout is None:
            timeout = self.gemma_timeout
            
        try:
            # Thử Ollama API trước
...
def test_router():
    ...
    # Test simple message
    simple_payload = {"message": "xin chào"}
    print("Testing simple message:")
    result = router.route_chat(simple_payload)
    print(f"Engine: {result.get('engine')}, Model: {result.get('model')}")
    print(f"Response: {result.get('response', '')[:100]}...")
    
    # Test complex message
    complex_payload = {"message": "Hãy viết đoạn code Python đọc CSV, tính trung bình và in kết quả theo cột."}
    print("\nTesting complex message:")
    result = router.route_chat(complex_payload)
    print(f"Engine: {result.get('engine')}, Model: {result.get('model')}")
    print(f"Response: {result.get('response', '')[:100]}...")
