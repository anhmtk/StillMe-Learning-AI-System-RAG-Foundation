...
from agent_dev.core.cleanup_manager import CleanupManager, CleanupOpportunity, CleanupType
...
class TestCleanupManager:
    ...
    def test_temp_file_detection(self):
        ...
        cleanup_result = self.cleanup_manager.analyze_cleanup_opportunities(self.temp_dir)
        
        assert isinstance(cleanup_result, list)
        assert len(cleanup_result) > 0
        
        # Should detect temp files
        temp_files = [item for item in cleanup_result if item.cleanup_type == CleanupType.TEMP_FILES]
        assert len(temp_files) > 0
        
        # Should not detect normal files
        normal_files = [item for item in cleanup_result if item.cleanup_type == CleanupType.TEMP_FILES and "normal_file.py" in item.file_path]
...
""")
        ...
        dead_code_items = [item for item in cleanup_result if item.cleanup_type == CleanupType.DEAD_CODE]
...
"""
        ...
        duplicate_items = [item for item in cleanup_result if item.cleanup_type == CleanupType.DUPLICATES]
    ...
    def test_cache_cleanup_detection(self):
        ...
        cache_items = [item for item in cleanup_result if item.cleanup_type == CleanupType.CACHE_CLEANUP]
...
""")
        ...
        optimization_items = [item for item in cleanup_result if item.cleanup_type == CleanupType.RESOURCE_OPTIMIZATION]
        ...
        assert "large" in optimization_text.lower() or "inefficient" in optimization_text.lower()
