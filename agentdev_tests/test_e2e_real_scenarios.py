...
""")
        ...
        assert "set(" in optimized_code or "dict(" in optimized_code  # Efficient data structures
        assert "**" in optimized_code  # Efficient power operation
        
        # Should remove inefficient nested loops
        assert "for i, item in enumerate(data):" not in optimized_code or "for j, other_item in enumerate(data):" not in optimized_code
...
""")
        ...
        assert "shell=False" in secured_code or "validate_command" in secured_code.lower()
...
""")
        ...
        assert "list comprehension" in refactored_code.lower() or "[" in refactored_code and "for" in refactored_code
        
        # Should add type hints
        assert "def process_data(data: List[int]) -> List[int]:" in refactored_code or "typing" in refactored_code
...
""")
        ...
        # Verify optimizations were applied
        with open(slow_app, 'r') as f:
            optimized_code = f.read()
        
        # Should use efficient algorithms
        assert "memoization" in optimized_code.lower() or "cache" in optimized_code.lower()
...
""")
        ...
        assert "ValueError" in improved_code or "FileNotFoundError" in improved_code or "KeyError" in improved_code
...
""")
        ...
        doc_files = [f for f in os.listdir(self.test_project) if f.endswith(('.md', '.rst', '.txt'))]
