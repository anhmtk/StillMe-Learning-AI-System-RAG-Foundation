...
class LearningOptimizationEngine:
    ...
    def _initialize_knowledge_base(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Initialized knowledge base with {len(default_knowledge)} entries")
    ...
    def _load_existing_knowledge(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Loaded {len(self.learning_entries)} learning entries and {len(self.optimization_results)} optimization results")
    ...
    def _setup_learning_monitoring(self):
        ...
        try:
            ...
            self.integration_bridge.register_endpoint(
                "GET", "/learning/knowledge", self._get_knowledge_base, auth_required=True
            )
            self.integration_bridge.register_endpoint(
                "GET", "/learning/optimization", self._get_optimization_results, auth_required=True
            )
            self.integration_bridge.register_endpoint(
                "POST", "/learning/optimize", self._trigger_optimization, auth_required=True
            )
            self.integration_bridge.register_endpoint(
                "GET", "/learning/insights", self._get_learning_insights, auth_required=True
            )
            ...
            for entry in entries:
                if "performance" in entry.input_data:
                    ...
                    if "status" in perf_data:
                        performance_values.append(1 if perf_data["status"] == "healthy" else 0)
            ...
            if len(self.learning_entries) >= 20:
                ...
                for entry in recent_entries:
                    if "performance" in entry.input_data:
                        perf_data = entry.input_data["performance"]
                        if "status" in perf_data:
                            performance_metrics.append(1 if perf_data["status"] == "healthy" else 0)
        ...
        try:
            ...
            result = OptimizationResult(
                ...
                parameters={
                    ...
                    "strategy": opportunity.get("strategy", "").value if hasattr(opportunity.get("strategy", ""), 'value') else str(opportunity.get("strategy", ""))
                },
                timestamp=datetime.now(),
        ...
        try:
            return {
                ...
                "data": {
                    ...
                    "performance_metrics": {
                        "avg_learning_time": sum(self.performance_metrics['learning_times']) / len(self.performance_metrics['learning_times']) if self.performance_metrics['learning_times'] else 0,
                        "avg_optimization_time": sum(self.performance_metrics['optimization_times']) / len(self.performance_metrics['optimization_times']) if self.performance_metrics['optimization_times'] else 0
                    }
                }
            }
        ...
        try:
            ...
            return {
                ...
                "data": {
                    ...
                    "categories": list(set(kb.category.value for kb in self.knowledge_base.values()))
                }
            }
    ...
    async def _get_optimization_results(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.optimization_results:
                avg_improvement = sum(r.improvement_percentage for r in self.optimization_results) / len(self.optimization_results)
            ...
            return {
                ...
                "data": {
                    ...
                    "best_improvement": max((r.improvement_percentage for r in self.optimization_results), default=0)
                }
            }
...
def main():
    ...
    async def test_learning_engine():
        ...
        print(f"Average improvement: {optimization['data']['average_improvement']:.1f}%")
