...
class SilentEvolutionaryLearningSystem:
    ...
    def start_silent_learning(self, proposal_id: str) -> str:
        ...
        try:
            ...
            thread = threading.Thread(
                target=self._silent_learning_process, args=(session_id, proposal_id)
            )
            thread.daemon = True
            thread.start()
    ...
    def _silent_learning_process(self, session_id: str, proposal_id: str):
        ...
        try:
            ...
            progress_per_objective = (
                100 / total_objectives if total_objectives > 0 else 100
            )
            ...
            for i, objective in enumerate(objectives):
                ...
                self.proposals_manager.update_proposal(
                    proposal_id, {"current_objective": i}
                )

                logger.info(
                    f"🎯 Learning objective {i+1}/{total_objectives}: {objective}"
                )
                ...
                while current_objective_progress < 100:
                    ...
                    overall_progress = (
                        objective_start_progress
                        + (current_objective_progress / 100) * progress_per_objective
                    )
    ...
    def _send_completion_notification(
        self, proposal_id: str, session_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            if proposal:
                ...
                logger.warning(
                    f"⚠️ Không thể lấy proposal để gửi notification: {proposal_id}"
                )
        except Exception as e:
    ...
    def _send_error_notification(self, proposal_id: str, error_message: str):
        ...
        try:
            logger.error(
                f"🚨 Learning error for proposal {proposal_id}: {error_message}"
            )
    ...
    def get_active_sessions_count(self) -> int:
        ...
        return len(
            [
                ...
                if s.get("status") == "silent_learning"
            ]
        )
