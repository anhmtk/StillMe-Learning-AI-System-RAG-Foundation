...
class AutoApprovalEngine:
                ...
                if self._meets_auto_approval_criteria(proposal):
                    if self._can_start_new_session():
                        ...
                        if success:
                            ...
                            logger.warning(f"⚠️ Failed to start learning for: {proposal.title}")
                    else:
                        logger.info(f"⏳ Queue proposal (max sessions reached): {proposal.title}")
                else:
    ...
    def _meets_auto_approval_criteria(self, proposal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if quality_score < self.quality_threshold:
                logger.debug(f"❌ Quality score too low: {quality_score} < {self.quality_threshold}")
                return False
            
            # Check duration
            estimated_duration = proposal.get('estimated_duration', 0)
            if estimated_duration > self.max_duration:
                logger.debug(f"❌ Duration too long: {estimated_duration} > {self.max_duration}")
                return False
            
            # Check learning objectives
            objectives = proposal.get('learning_objectives', [])
            if len(objectives) < self.min_objectives:
                logger.debug(f"❌ Too few objectives: {len(objectives)} < {self.min_objectives}")
                return False
            
            # Check for duplicates
            if self._is_duplicate(proposal):
                logger.debug(f"❌ Duplicate proposal: {proposal.get('title', 'Unknown')}")
                return False
            
            logger.debug(f"✅ Proposal meets criteria: {proposal.get('title', 'Unknown')}")
            return True
    ...
    def _auto_approve_and_start(self, proposal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Approve proposal
            success = self.proposals_manager.approve_proposal(proposal_id, "auto_approval_system")
            if not success:
            ...
            logger.info(f"🎯 Đã bắt đầu silent learning: {proposal.get('title', 'Unknown')}")
            return True
    ...
    def run_approval_cycle(self) -> int:
        ...
        logger.info(f"✅ Hoàn thành auto-approval cycle: {approved_count} proposals approved")
        return approved_count
