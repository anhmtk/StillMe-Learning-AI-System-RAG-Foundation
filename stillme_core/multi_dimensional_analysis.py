...
class MultiDimensionalAnalysis:
    ...
    def __init__(self, 
        ...
        logger.info("✅ MultiDimensionalAnalysis initialized với enterprise-grade configuration")
    ...
    def analyze_time_series(self, 
                          ...
                          granularity: str = 'hour') -> MultiDimensionalResult:
        """
        ...
        try:
            ...
            time_series_data = self._get_time_series_data(dimension, time_range_hours, granularity)
            
            # Perform statistical analysis
            statistical_analysis = self._perform_statistical_analysis(time_series_data)
            
            # Generate insights
            insights = self._generate_time_series_insights(time_series_data, statistical_analysis)
            
            # Generate recommendations
            recommendations = self._generate_time_series_recommendations(statistical_analysis)
            
            # Calculate confidence score
            confidence_score = self._calculate_confidence_score(time_series_data, statistical_analysis)
            
            # Create result
            result = MultiDimensionalResult(
                analysis_type="time_series",
                dimensions=[dimension],
                time_range=(datetime.now() - timedelta(hours=time_range_hours), datetime.now()),
            ...
            logger.debug(f"📊 Time series analysis completed for {dimension} in {analysis_duration:.1f}ms")
    ...
    def analyze_user_behavior(self, 
                            time_range_hours: int = 168) -> Dict[str, UserBehaviorMetrics]:
        """
        ...
        try:
            ...
            for user_id, session_id, event_count, avg_duration, first_event, last_event, unique_features in user_sessions:
            ...
            for user_id, sessions in user_sessions_dict.items():
                ...
                avg_session_duration = statistics.mean([s['avg_duration'] for s in sessions])
                ...
                engagement_score = self._calculate_engagement_score(sessions, feature_usage_frequency)
                
                # Calculate retention rate
                retention_rate = self._calculate_retention_rate(sessions, time_range_hours)
            ...
            logger.debug(f"📊 User behavior analysis completed for {len(user_metrics)} users in {analysis_duration:.1f}ms")
    ...
    def analyze_feature_usage_patterns(self, 
                                     time_range_hours: int = 168) -> Dict[str, FeatureUsagePattern]:
        """
        ...
        try:
            ...
            for feature_name, usage_count, unique_users, avg_duration, success_rate in feature_data:
                ...
                usage_trend = self._calculate_usage_trend(feature_name, time_range_hours)
                
                # Get peak usage hours
                peak_usage_hours = self._get_peak_usage_hours(hourly_usage_dict[feature_name])
                
                # Calculate correlation with other features
                correlation_with_other_features = self._calculate_feature_correlation(feature_name, time_range_hours)
            ...
            logger.debug(f"📊 Feature usage pattern analysis completed for {len(feature_patterns)} features in {analysis_duration:.1f}ms")
    ...
    def _get_time_series_data(self, dimension: str, time_range_hours: int, granularity: str) -> List[TimeSeriesData]:
    ...
    def _perform_statistical_analysis(self, time_series_data: List[TimeSeriesData]) -> Dict[str, Any]:
    ...
    def _detect_seasonality(self, values: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            seasonality_score = statistics.mean(hourly_variance) / max(0.001, statistics.mean(values))
    ...
    def _generate_time_series_insights(self, time_series_data: List[TimeSeriesData], statistical_analysis: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if volatility > 0.5:
                insights.append("High volatility detected - usage patterns are inconsistent")
            elif volatility < 0.1:
            ...
            if seasonality.get('detected', False):
                insights.append(f"Seasonal pattern detected: {seasonality.get('pattern', 'unknown')}")
    ...
    def _generate_time_series_recommendations(self, statistical_analysis: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            trend = statistical_analysis.get('trend', 'unknown')
            if trend == 'increasing':
                recommendations.append("Consider scaling resources to handle increased demand")
            elif trend == 'decreasing':
            ...
            volatility = statistical_analysis.get('volatility', 0)
            if volatility > 0.5:
                recommendations.append("Implement load balancing to handle usage spikes")
            
            # Seasonality-based recommendations
            seasonality = statistical_analysis.get('seasonality', {})
            if seasonality.get('detected', False):
                recommendations.append("Implement seasonal resource allocation strategies")
    ...
    def _calculate_confidence_score(self, time_series_data: List[TimeSeriesData], statistical_analysis: Dict[str, Any]) -> float:
        ...
        try:
            ...
            base_confidence = min(1.0, data_points / 100.0)  # 100 data points = 100% confidence
            
            # Adjust for data quality
            volatility = statistical_analysis.get('volatility', 0)
            quality_factor = max(0.5, 1.0 - volatility)  # Lower volatility = higher quality
    ...
    def _calculate_usage_patterns(self, sessions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            session_frequency = len(sessions) / 7.0  # sessions per day (assuming 1 week)
    ...
    def _calculate_engagement_score(self, sessions: List[Dict[str, Any]], feature_usage: Dict[str, int]) -> float:
        ...
        try:
            ...
            engagement_score = (frequency_score * 0.4 + diversity_score * 0.3 + duration_score * 0.3)
    ...
    def _calculate_retention_rate(self, sessions: List[Dict[str, Any]], time_range_hours: int) -> float:
    ...
    def _get_peak_usage_hours(self, hourly_usage: Dict[int, int]) -> List[int]:
        ...
        try:
            ...
            peak_hours = [hour for hour, usage in hourly_usage.items() if usage >= threshold]
    ...
    def _calculate_feature_correlation(self, feature_name: str, time_range_hours: int) -> Dict[str, float]:
        """Calculate correlation with other features"""
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                since_time = datetime.now() - timedelta(hours=time_range_hours)
                ...
                for other_feature, other_usage in feature_hourly_usage.items():
                    if other_feature != feature_name:
                        correlation = self._calculate_correlation(target_usage, other_usage)
    ...
    def _calculate_correlation(self, usage1: Dict[str, int], usage2: Dict[str, int]) -> Optional[float]:
    ...
    def _calculate_business_impact_score(self, usage_count: int, user_adoption_rate: float, success_rate: float, avg_duration: float) -> float:
        ...
        try:
            ...
            impact_score = (usage_score * 0.3 + adoption_score * 0.3 + success_score * 0.2 + efficiency_score * 0.2)
    ...
    def get_analysis_summary(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'performance_metrics': {
                    'avg_analysis_time_ms': statistics.mean(self._analysis_times) if self._analysis_times else 0.0,
...
# Factory function
def create_multi_dimensional_analysis(metrics_db_path: str, config: Optional[Dict[str, Any]] = None) -> MultiDimensionalAnalysis:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--user-behavior", action="store_true", help="Analyze user behavior")
    parser.add_argument("--feature-patterns", action="store_true", help="Analyze feature patterns")
