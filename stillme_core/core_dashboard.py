...
class CoreDashboard:
    ...
    async def _get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                health_score = min(100.0, (success_rate * 100) - (avg_response_time / 100.0))
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                if module_performance:
                    overall_avg_duration = statistics.mean([m['avg_duration_ms'] for m in module_performance])
                    overall_success_rate = statistics.mean([m['success_rate'] for m in module_performance])
    ...
    async def _get_value_metrics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                roi_percentage = ((cost_savings_usd - estimated_investment) / max(1, estimated_investment)) * 100.0
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                return {
                    ...
                    'most_used_module': module_usage[0]['module_name'] if module_usage else 'None',
                    'last_updated': datetime.now().isoformat()
                }
    ...
    async def _generate_recommendations(self, 
                                      ...
                                      performance_metrics: Dict[str, Any],
                                      value_metrics: Dict[str, Any],
                                      usage_statistics: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if system_health.get('health_score', 0) < 80:
                recommendations.append("System health is below optimal. Consider investigating performance issues.")
            ...
            if value_metrics.get('roi_percentage', 0) < 0:
                recommendations.append("ROI is negative. Review cost optimization strategies.")
            
            # Usage-based recommendations
            if usage_statistics.get('total_usage_24h', 0) < 100:
                recommendations.append("Low usage detected. Consider user engagement strategies.")
    ...
    async def _check_performance_alerts(self, metrics: DashboardMetrics):
        ...
        try:
            ...
            if avg_duration > self._alert_thresholds['performance']['response_time_critical']:
            ...
            elif avg_duration > self._alert_thresholds['performance']['response_time_warning']:
                await self._create_alert(
                    severity='warning',
                    category='performance',
            ...
            if success_rate < self._alert_thresholds['performance']['error_rate_critical']:
                await self._create_alert(
                    severity='critical',
                    category='performance',
            ...
            elif success_rate < self._alert_thresholds['performance']['error_rate_warning']:
                await self._create_alert(
                    severity='warning',
                    category='performance',
    ...
    def get_dashboard_data(self, time_range_hours: int = 1) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                # Get recent metrics
                recent_metrics = [m for m in self._dashboard_metrics 
                ...
                return {
                    ...
                    'metrics_history': [asdict(m) for m in recent_metrics[-10:]],  # Last 10 updates
                    'alerts': [asdict(alert) for alert in self._alerts if not alert.resolved],
    ...
    def get_visualization_data(self, 
                              chart_type: str,
                              time_range_hours: int = 24) -> VisualizationData:
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                return VisualizationData(
                    ...
                    time_range=f"{time_range_hours}h",
                    metadata={'total_savings': sum([d['cost_savings_usd'] for d in data])}
                )
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                latest_metrics = self._dashboard_metrics[-1] if self._dashboard_metrics else None
                
                if not latest_metrics:
                    return {'error': 'No metrics available'}
                
                return {
                    'system_status': latest_metrics.system_health.get('status', 'unknown'),
                    'health_score': latest_metrics.system_health.get('health_score', 0.0),
                    'active_alerts': len([a for a in self._alerts if not a.resolved]),
                    'total_events_24h': latest_metrics.usage_statistics.get('total_usage_24h', 0),
                    'roi_percentage': latest_metrics.value_metrics.get('roi_percentage', 0.0),
...
# Factory function
def create_dashboard(metrics_db_path: str, 
                    validation_db_path: str,
                    config: Optional[Dict[str, Any]] = None) -> CoreDashboard:
