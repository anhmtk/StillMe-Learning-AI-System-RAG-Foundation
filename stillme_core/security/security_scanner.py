...
class SecurityScanner:
    ...
    def _run_bandit_scan(self, include_tests: bool) -> List[SecurityIssue]:
        ...
        try:
            ...
            result = subprocess.run(args, cwd=self.repo_root, 
            ...
            if result.returncode in [0, 1]:  # 0 = no issues, 1 = issues found
                ...
                for issue in data.get("results", []):
                    security_issue = SecurityIssue(
                        tool="bandit",
                        level=VulnerabilityLevel(issue.get("issue_severity", "low").lower()),
        ...
        try:
            ...
            if result.returncode in [0, 1]:  # 0 = no issues, 1 = issues found
                ...
                for result_item in data.get("results", []):
                    ...
                    security_issue = SecurityIssue(
                        tool="semgrep",
                        level=severity_map.get(result_item.get("extra", {}).get("severity", "INFO"), 
    ...
    def _generate_security_report(self, issues: List[SecurityIssue], duration: float) -> SecurityReport:
        """Generate comprehensive security report"""
        # Count issues by level
        issues_by_level = {level: 0 for level in VulnerabilityLevel}
        for issue in issues:
    ...
    def _generate_recommendations(self, issues: List[SecurityIssue], 
                                issues_by_level: Dict[VulnerabilityLevel, int]) -> List[str]:
        ...
        if len(issues) == 0:
            recommendations.append("✅ No security issues detected. Keep up the good work!")
        elif len(issues) < 5:
            recommendations.append("👍 Good security posture. Address remaining issues for better security.")
        else:
