...
class PatchExecutor:
    ...
    def create_feature_branch(self, name: str) -> ExecResult:
        ...
        return _run(
            ["git", "checkout", "-b", name], cwd=self._ensure_sandbox(), timeout=10
        )
            ...
            if not self._sandbox_path:
                ...
                if sandbox_dir:
                    self._sandbox_path = str(
                        prepare_sandbox(self.repo_root, sandbox_dir)
                    )
    ...
    def push_branch(self, remote: str = "origin") -> tuple[bool, str]:
        try:
            ...
            res = _run(
                ["git", "push", "-u", remote, branch], cwd=self._ensure_sandbox()
            )
        ...
        try:
            ...
            if not ok or not owner or not repo:
                return {
                    ...
                    "number": None,
                    "provider": None,
                    "error": err or "remote_unavailable",
                }
            ...
            if shutil.which("gh"):
                ...
                if draft:
                    args.append("--draft")
                p = subprocess.run(
                    args, cwd=self._ensure_sandbox(), capture_output=True, text=True
                )
                if p.returncode == 0:
