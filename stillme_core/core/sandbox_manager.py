...
class DockerSandboxManager:
    def __init__(
        self, project_path: str = ".", docker_image: str = "python:3.9-slim-buster"
    ):
    ...
    def start_container(self) -> bool:  # type: ignore
        ...
        result = self._run_command_in_host(
        ...
        cmd = [
            ...
            self.container_name,
            "-v",
            f"{self.project_path}:/app",  # Mount project directory
            "-w",
            "/app",  # Set working directory inside container
            self.docker_image,
            "tail",
            "-f",
            "/dev/null",
        ...
        logger.info(
            f"Container {self.container_name} started with ID: {self.container_id}"
        )
        return True
    ...
    def run_command(
        ...
        if not self.container_id:
            ...
            return subprocess.CompletedProcess(
                args=command, returncode=1, stdout="", stderr="No active container."
            )
        ...
        try:
            ...
            return subprocess.CompletedProcess(
                args=command, returncode=1, stdout="", stderr="Command timed out."
            )
        except Exception as e:
    ...
    def get_file_content(self, file_path_in_container: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"Failed to get file content from {file_path_in_container}: {result.stderr}"
                )
                return None
        except Exception as e:
    ...
    def write_file_content(self, file_path_in_container: str, content: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"Failed to write file content to {file_path_in_container}: {result.stderr}"
                )
                return False
        except Exception as e:
