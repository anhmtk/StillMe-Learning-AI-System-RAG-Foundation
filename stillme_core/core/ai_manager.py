...
def dev_agent(task: str, mode: str = DEFAULT_MODE, **params: Any) -> str:
    ...
    Router tác vụ:
      ...
      - real  : AgentDev thực sự với feedback chi tiết
    """
    ...
    if any(
        ...
        for keyword in ["lỗi", "error", "sửa", "fix", "code", "viết", "test", "build"]
    ):
        try:
...
class AIManager:
    ...
    def get_context_aware_plan(self, goal: str, max_items: int = 5) -> list[PlanItem]:
        ...
        try:
            ...
            prompt = self._build_context_aware_prompt(
                goal, context_info, learning_insights, max_items
            )
    ...
    def _get_learning_insights(self, goal: str) -> dict[str, Any]:
        ...
        insights = {"similar_bugs": [], "success_strategies": [], "risk_factors": []}
...
"""
    ...
    def _get_ai_response_with_fallback(self, prompt: str, mode: str = "fast") -> str:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                current_mode = self.fallback_strategy.get_fallback_mode(
                    current_mode, str(e)
                )
    ...
    async def aget_ai_response(
        self, prompt: str, mode: str = "fast", **params: Any
    ) -> str:
    ...
    def generate_patch(self, plan_item: PlanItem, context: str = "") -> str:
        ...
        if provider == "LOCAL_HTTP":
            endpoint = os.getenv(
                "LOCAL_MODEL_ENDPOINT", "http://127.0.0.1:8000/v1/chat/completions"
            )
            try:
                r = httpx.post(
                    endpoint,
