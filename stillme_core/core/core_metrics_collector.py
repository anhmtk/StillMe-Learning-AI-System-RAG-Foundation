...
class CoreMetricsCollector:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_usage_module ON usage_events(module_name)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_perf_timestamp ON performance_metrics(timestamp)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_perf_module ON performance_metrics(module_name)"
                )
    ...
    def track_usage(
        ...
        try:
            ...
            logger.debug(
                f"📊 Usage tracked: {module_name}.{feature_name} ({duration_ms}ms)"
            )
            return event_id
    ...
    def calculate_value_metrics(self, time_range_hours: int = 24) -> ValueMetrics:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                roi_percentage = (
                    cost_savings_usd / max(1, time_range_hours * 10.0)
                ) * 100.0
    ...
    def cleanup_old_data(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute(
                    "DELETE FROM value_metrics WHERE timestamp < ?",
                    [cutoff_date.isoformat()],
                )
                ...
                logger.info(
                    f"🧹 Cleaned up {deleted_events} events, {deleted_metrics} metrics, {deleted_values} values"
                )
    ...
    def get_current_usage(self) -> dict[str, float]:
        ...
        try:
            return {
                ...
                "disk_io_read": (
                    self._process.io_counters().read_bytes
                    if hasattr(self._process, "io_counters")
                    else 0
                ),
                "disk_io_write": (
                    self._process.io_counters().write_bytes
                    if hasattr(self._process, "io_counters")
                    else 0
                ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate accuracy report"
    )
