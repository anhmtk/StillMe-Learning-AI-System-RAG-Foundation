...
class FinancialValidationEngine:
    ...
    async def validate_financial_calculation(
        ...
        try:
            logger.info(
                f"Đang xác thực tính toán tài chính: {calc_data['calculation_type']}"
            )
            errors = []
            ...
            validation_result = ValidationResult(
                is_valid=is_valid, errors=errors, confidence_score=confidence_score
            )
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO financial_calculations VALUES (NULL, ?, ?, ?, ?, ?)",
                (
                    financial_calc.amount,
                    financial_calc.currency,
                    financial_calc.calculation_type,
                    financial_calc.formula,
                    financial_calc.result,
                ),
            )
            cursor.execute(
                "INSERT INTO validation_results VALUES (NULL, ?, ?, ?, ?)",
                (
    ...
    async def check_compliance(
        ...
        try:
            logger.info(
                f"Đang kiểm tra tuân thủ quy định: {compliance_data['regulation_type']}"
            )
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO compliance_checks VALUES (NULL, ?, ?, ?, ?, ?)",
                (
                    compliance_check.regulation_type,
                    compliance_check.requirement,
                    compliance_check.status,
                    compliance_check.evidence,
                    compliance_check.risk_score,
                ),
            )
            conn.commit()
            conn.close()
    ...
    async def generate_audit_trail(
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO audit_trails VALUES (NULL, ?, ?, ?, ?, ?)",
                (
                    audit_trail.action,
                    audit_trail.user,
                    audit_trail.timestamp,
                    json.dumps(audit_trail.details),
                    audit_trail.hash,
                ),
            )
            conn.commit()
    ...
    async def create_regulatory_report(
        ...
        try:
            logger.info(
                f"Đang tạo báo cáo tuân thủ: {report_type} cho giai đoạn {period}"
            )

            # Giả lập logic kiểm tra tuân thủ để tạo báo cáo
            compliance_status = (
                "Đã tuân thủ"
                if data.get("sox_status") == "compliant"
                else "Không tuân thủ"
            )
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO regulatory_reports VALUES (NULL, ?, ?, ?, ?)",
                (
                    regulatory_report.report_type,
                    regulatory_report.period,
                    json.dumps(regulatory_report.data),
                    regulatory_report.compliance_status,
                ),
            )
            conn.commit()
            conn.close()
    ...
    async def validate_pricing_accuracy(
        self, pricing_engine: Any, product_id: str
    ) -> ValidationResult:
        """
        ...
        try:
            logger.info(
                f"Đang xác thực độ chính xác của giá cho sản phẩm: {product_id}"
            )
            ...
            pricing_recommendation = (
                await pricing_engine.generate_pricing_recommendation(product_id)
            )
            ...
            # So sánh
            is_valid = abs(calculated_price - external_price) / external_price < (
                1 - self.config["validation_threshold"]
            )
...
async def main():
    try:
        ...
        audit_trail = await validation_engine.generate_audit_trail(
            "data_modification", "admin_user", {"old_value": "100", "new_value": "120"}
        )
        print(f"\nAudit Trail Hash: {audit_trail.hash}")

        # 4. Tạo báo cáo tuân thủ
        report_data = {"sox_status": "compliant", "data_integrity": "high"}
        regulatory_report = await validation_engine.create_regulatory_report(
            "Quarterly Report", "Q3 2025", report_data
        )
