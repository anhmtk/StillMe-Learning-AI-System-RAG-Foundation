...
class EnhancedExecutor:
    ...
    def analyze_test_impact(self, changed_files: list[str]) -> TestImpact:
        ...
        impact_percent = (
            (len(affected_tests) / total_tests * 100) if total_tests > 0 else 0
        )
    ...
    def generate_coverage_report(
        ...
        coverage_values = [
            r.coverage_percent for r in test_results if r.coverage_percent is not None
        ]
        overall_coverage = (
    ...
    def _generate_coverage_recommendations(
        ...
        if coverage < 70:
            ...
            recommendations.append(
                "Coverage is below 80%. Consider improving test coverage."
            )

        if failed_tests > 0:
            recommendations.append(
                f"{failed_tests} tests failed. Review and fix failing tests."
            )
