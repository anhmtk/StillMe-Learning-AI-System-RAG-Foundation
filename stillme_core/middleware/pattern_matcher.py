...
class PatternMatcher:
        ...
        if not AHO_AVAILABLE or not self.patterns:
            ...
            for pattern in self.patterns:
                if pattern["type"] == "literal":
                    ...
                    regex_pattern = re.compile(f"\\b{escaped_text}\\b", re.IGNORECASE | re.UNICODE)
                    self.regex_patterns.append({
                        "id": pattern["id"],
    ...
    def match(self, text: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        match_time = (time.perf_counter() - start_time) * 1_000_000  # Convert to microseconds
    ...
    def _calculate_pattern_score(self, matches: List[Dict[str, Any]], text_length: int) -> float:
        ...
        coverage = sum(end - start for start, end in [match["span"] for match in matches]) / max(text_length, 1)
