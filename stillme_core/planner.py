...
def _normalize_plan_v1(raw: Dict[str, Any], problem_file: Optional[str]) -> Dict[str, Any]:
    """
    ...
    """
    module_name = (raw.get("module_name") or (Path(problem_file).stem if problem_file else "unknown_module"))
    ...
    for idx, s in enumerate(steps_in, 1):
        ...
        reasoning = s.get("reasoning") or s.get("code") or s.get("explanation") or s.get("text") or ""
        steps_out.append({
...
class Planner:
    ...
    def _try_parse_and_validate_plan(
        ...
        try:
            ...
            try:
                ...
                data = {"module_name": Path(problem_file).stem if problem_file else "unknown_module",
                        "steps": []}
            ...
            logger.error(f"❌ Lỗi validate plan: {str(e)}")
            return None

    def _build_structured_prompt(self, problem_desc: str, strict_json: bool = False) -> str:
    ...
    def build_plan(self, max_items: int = 5) -> List[PlanItem]:
        ...
        try:
            ...
            for file_path, _cnt in sorted(stats.items(), key=lambda kv: kv[1], reverse=True):
        ...
        if len(items) < 2:
            items.append(PlanItem(id=f"FB-{len(items)+1}", title="Run focused tests", action="run_tests", tests_to_run=["tests/"]))
        if len(items) < 2:
            items.append(PlanItem(id=f"FB-{len(items)+1}", title="Lint key files", action="command", tests_to_run=["tests/"]))
    ...
    def _build_prompt(
        ...
        prompt += "🎯 Yêu cầu:\n"
        prompt += "1. Phân tích nguyên nhân\n2. Đề xuất giải pháp\n3. Code mẫu (nếu cần)\n"
        return prompt
