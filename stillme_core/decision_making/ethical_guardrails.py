...
class EthicalGuardrails:
    ...
    def assess_decision(self, decision_data: Dict[str, Any], 
                       context: Dict[str, Any]) -> EthicalAssessment:
        """
        ...
        if assessment_id in self.assessment_cache:
            ...
            if time.time() - cached_assessment.assessment_details.get('timestamp', 0) < 3600:  # 1 hour cache
        ...
        is_ethical = len([v for v in violations if v.severity in [ViolationSeverity.SEVERE, ViolationSeverity.CRITICAL]]) == 0
    ...
    def _check_boundary(self, boundary: EthicalBoundary, 
        ...
        if exceptions_apply:
            result["recommendation"] = f"Exception applies: {', '.join(exceptions_apply)}"
            return result
        
        # Violation detected
        result["violated"] = True
        result["description"] = f"{boundary.description} - Conditions met: {', '.join(conditions_met)}"
        ...
        if boundary.enforcement_action == "block_decision":
            result["recommendation"] = "Decision should be blocked due to ethical violation"
    ...
    def _calculate_confidence_score(self, violations: List[EthicalViolation], 
    ...
    def _generate_assessment_id(self, decision_data: Dict[str, Any], 
    ...
    def _save_configuration(self):
        ...
        config = {
            ...
            "violation_history": [asdict(violation) for violation in self.violation_history[-1000:]],  # Keep last 1000
    ...
    def _generate_ethical_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats["total_violations"] > 10:
            recommendations.append("Consider reviewing ethical boundaries - high violation count")
        ...
        if stats["recent_violations"] > 5:
            recommendations.append("Recent increase in violations - review decision processes")
