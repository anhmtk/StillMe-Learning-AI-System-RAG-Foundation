...
class StillMeEcosystemDiscovery:
    ...
    def _scan_directory(self, directory: Path, module_type: str) -> Dict[str, ModuleInfo]:
    ...
    def _analyze_module(self, file_path: Path, module_type: str) -> Optional[ModuleInfo]:
        ...
        try:
            ...
            health_score = self._calculate_health_score(content, function_count, class_count, len(imports))
    ...
    def _calculate_health_score(self, content: str, function_count: int, class_count: int, import_count: int) -> float:
    ...
    def calculate_complexity_metrics(self) -> Dict[str, Any]:
        ...
        if self.modules:
            ...
            total_deps = sum(len(module.dependencies) for module in self.modules.values())
    ...
    def _check_alerts(self):
        ...
        if self.health_metrics.get("memory_usage", 0) > self.alert_thresholds["memory_usage"]:
    ...
    def generate_ecosystem_report(self) -> Dict[str, Any]:
        ...
        for module in self.modules.values():
            if module.health_score < 50:
                critical_issues.append(f"Module {module.name} has low health score: {module.health_score:.1f}")
        ...
        for module in self.modules.values():
            if module.performance_metrics.get("import_time_ms", 0) > 100:
                performance_bottlenecks.append(f"Module {module.name} has slow import time: {module.performance_metrics['import_time_ms']:.1f}ms")
        ...
        report = {
            ...
            "total_modules": len(self.modules),
            "core_modules_found": len([m for m in self.modules.values() if m.type == "core"]),
            "extension_modules": len([m for m in self.modules.values() if m.type == "extension"]),
            "critical_issues": critical_issues,
            "performance_bottlenecks": performance_bottlenecks,
            "circular_dependencies": circular_deps,
            "complexity_metrics": complexity_metrics,
            "health_metrics": self.health_metrics,
            "module_details": {name: asdict(module) for name, module in self.modules.items()},
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if low_health_modules:
            recommendations.append(f"Improve health of {len(low_health_modules)} modules with low health scores")
        
        # Security recommendations
        modules_with_security_issues = [m for m in self.modules.values() if m.security_issues]
        if modules_with_security_issues:
            recommendations.append(f"Address security issues in {len(modules_with_security_issues)} modules")
        
        # Performance recommendations
        slow_modules = [m for m in self.modules.values() if m.performance_metrics.get("import_time_ms", 0) > 50]
        if slow_modules:
            recommendations.append(f"Optimize performance of {len(slow_modules)} slow modules")
        
        # Documentation recommendations
        poorly_documented = [m for m in self.modules.values() if m.documentation_quality < 50]
        if poorly_documented:
            recommendations.append(f"Improve documentation for {len(poorly_documented)} modules")
