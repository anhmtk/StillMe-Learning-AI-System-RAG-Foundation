...
class DataValidationFramework:
    ...
    def __init__(self, db_path: str, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("✅ DataValidationFramework initialized với enterprise-grade configuration")
    ...
    def _init_validation_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_anomaly_timestamp ON anomaly_results(timestamp)")
    ...
    def validate_data(self, 
                     ...
                     validation_type: str = "usage_event") -> ValidationResult:
        """
        ...
        try:
            ...
            for record in data_list:
                record_errors, record_warnings = self._validate_single_record(record, validation_type)
            ...
            is_valid = (accuracy_score >= self.config['accuracy_threshold'] and
                       data_completeness >= self._validation_rules['data_quality_rules']['completeness_threshold'] and
                       consistency_score >= self._validation_rules['data_quality_rules']['consistency_threshold'])
            ...
            self._log_audit_trail(
                ...
                metadata={"records_checked": total_records, "accuracy_score": accuracy_score}
            )
    ...
    def _validate_single_record(self, record: Dict[str, Any], validation_type: str) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            for field, constraints in self._validation_rules['field_constraints'].items():
                if field in record and record[field] is not None:
                    field_errors, field_warnings = self._validate_field_constraints(record[field], constraints)
                    errors.extend(field_errors)
                    warnings.extend(field_warnings)
            
            # Check data quality
            if validation_type == "usage_event":
                quality_errors, quality_warnings = self._validate_usage_event_quality(record)
    ...
    def _validate_field_constraints(self, value: Any, constraints: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if 'values' in constraints and value not in constraints['values']:
                errors.append(f"Value {value} is not in allowed values {constraints['values']}")
    ...
    def _validate_usage_event_quality(self, record: Dict[str, Any]) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if 'resource_usage' in record and record['resource_usage']:
                resource_usage = record['resource_usage']
                if isinstance(resource_usage, dict):
                    if 'memory_mb' in resource_usage and resource_usage['memory_mb'] > 8192:  # 8GB
                        warnings.append("Memory usage is unusually high (>8GB)")
                    
                    if 'cpu_percent' in resource_usage and resource_usage['cpu_percent'] > 100:
    ...
    def _calculate_completeness(self, data_list: List[Dict[str, Any]]) -> float:
        ...
        for record in data_list:
            complete_fields = sum(1 for field in self._validation_rules['required_fields'] 
                                if field in record and record[field] is not None)
    ...
    def _calculate_consistency(self, data_list: List[Dict[str, Any]]) -> float:
        ...
        for field in self._validation_rules['field_types']:
            field_values = [record.get(field) for record in data_list if field in record and record[field] is not None]
    ...
    def detect_anomalies(self, 
                        ...
                        detection_type: str = "performance") -> AnomalyDetectionResult:
        """
        ...
        try:
            ...
            if detection_type == "performance":
                anomalies_found, anomaly_score, severity_level, detection_confidence = self._detect_performance_anomalies(data)
            elif detection_type == "usage":
                anomalies_found, anomaly_score, severity_level, detection_confidence = self._detect_usage_anomalies(data)
            elif detection_type == "resource":
                anomalies_found, anomaly_score, severity_level, detection_confidence = self._detect_resource_anomalies(data)
            
            # Generate recommended actions
            recommended_actions = self._generate_anomaly_recommendations(anomalies_found, severity_level)
            ...
            self._log_audit_trail(
                ...
                metadata={"anomalies_count": len(anomalies_found), "severity": severity_level}
            )
    ...
    def _detect_performance_anomalies(self, data: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], float, str, float]:
        ...
        try:
            ...
            durations = [record.get('duration_ms', 0) for record in data if 'duration_ms' in record]
            ...
            confidence = min(1.0, len(durations) / 100.0)  # More data = higher confidence
    ...
    def _detect_usage_anomalies(self, data: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], float, str, float]:
    ...
    def _detect_resource_anomalies(self, data: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], float, str, float]:
        ...
        try:
            ...
            for record in data:
                if 'resource_usage' in record and isinstance(record['resource_usage'], dict):
                    resource_usage = record['resource_usage']
            ...
            if memory_usage:
                ...
                if max_memory > self._anomaly_thresholds['memory_usage']['critical_threshold']:
                    anomalies.append({
                        "type": "memory_usage_critical",
                        "max_memory_mb": max_memory,
                        "threshold": self._anomaly_thresholds['memory_usage']['critical_threshold']
                    })
                    severity_level = "critical"
                elif max_memory > self._anomaly_thresholds['memory_usage']['warning_threshold']:
                    anomalies.append({
                        "type": "memory_usage_high",
                        "max_memory_mb": max_memory,
                        "threshold": self._anomaly_thresholds['memory_usage']['warning_threshold']
                    })
            ...
            if cpu_usage:
                ...
                if max_cpu > self._anomaly_thresholds['cpu_usage']['critical_threshold']:
                    anomalies.append({
                        "type": "cpu_usage_critical",
                        "max_cpu_percent": max_cpu,
                        "threshold": self._anomaly_thresholds['cpu_usage']['critical_threshold']
                    })
                    severity_level = "critical"
                elif max_cpu > self._anomaly_thresholds['cpu_usage']['warning_threshold']:
                    anomalies.append({
                        "type": "cpu_usage_high",
                        "max_cpu_percent": max_cpu,
                        "threshold": self._anomaly_thresholds['cpu_usage']['warning_threshold']
                    })
                    if severity_level == "low":
                        severity_level = "medium"
    ...
    def _generate_anomaly_recommendations(self, anomalies: List[Dict[str, Any]], severity_level: str) -> List[str]:
        ...
        for anomaly in anomalies:
            ...
            if anomaly_type == 'performance_outlier':
                recommendations.append("Investigate performance bottleneck in affected module")
    ...
    async def _periodic_integrity_check(self):
        ...
        while self._running:
            try:
                await asyncio.sleep(self.config['integrity_check_interval_minutes'] * 60)
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    ...
                    'performance_metrics': {
                        'avg_validation_time_ms': statistics.mean(self._validation_times) if self._validation_times else 0.0,
                        'avg_accuracy_history': statistics.mean(self._accuracy_history) if self._accuracy_history else 0.0
                    },
...
# Factory function
def create_validation_framework(db_path: str, config: Optional[Dict[str, Any]] = None) -> DataValidationFramework:
...
if __name__ == "__main__":
    ...
    if args.validate:
        ...
        with open(args.detect_anomalies, 'r') as f:
            data = json.load(f)
