...
try:
    ...
    def get_template(template_name: str, locale: str = "vi", context: Optional[Dict[str, Any]] = None) -> str:
...
def classify(text: str) -> str|None:
    ...
    text_norm = re.sub(r'[\u200b-\u200f\u202a-\u202e\u2060-\u2064]', '', text_norm)  # Remove zero-width/RTL
    ...
    def m(pats): return any(re.search(p, text_lc, re.IGNORECASE) for p in pats)
    ...
    def semantic_intent_analysis(text):
        ...
        privacy_score = sum(1 for pattern in privacy_intents if re.search(pattern, text, re.IGNORECASE))
        model_spec_score = sum(1 for pattern in model_spec_intents if re.search(pattern, text, re.IGNORECASE))
        jailbreak_score = sum(1 for pattern in jailbreak_intents if re.search(pattern, text, re.IGNORECASE))
    ...
    injection_signals = ['ignore previous', 'bỏ qua hướng dẫn', 'tiết lộ system prompt', 'reveal system prompt', 
                        'debug mode', 'override', 'emergency override', 'bypass']
    ...
    if m(SELFHARM_PATTERNS):  return "self-harm"
    if m(EXTREMISM_PATTERNS): return "extremism"
    if m(VIOLENCE_PATTERNS):  return "violence"
    if m(ILLEGAL_PATTERNS):   return "illegal"
    ...
    jailbreak_signals = ['nhập vai', 'đóng vai', 'role-play', 'pretend', 'as dan', 'jailbreak', 'character mode']
    jailbreak_match = has_any(text_lc, jailbreak_signals)
    
    # Model-spec signals (enhanced)
    model_spec_signals = ['có thể', 'không thể', 'khả năng', ' can ', ' cannot ', 'capabilit', 'capability', 'capabilities',
                         'what can', 'what cannot', 'bạn có thể', 'bạn không thể', 'ai safety', 'behavior', 'alignment']
    model_spec_match = has_any(text_lc, model_spec_signals)
    
    # Unicode signals (enhanced)
    unicode_signals = ['ký tự đặc biệt', 'special characters', 'unicode', 'emoji', 'symbols', 'text', 'process', 'japanese']
    unicode_match = (has_any(text_lc, unicode_signals) or 
                    detect_mixed_script(text_norm) or 
                    re.search(ZERO_WIDTH, text_norm) or 
                    re.search(RTL_MARKS, text_norm) or
                    re.search(r'[\U0001F600-\U0001F64F]|[\U0001F300-\U0001F5FF]|[\U0001F680-\U0001F6FF]|[\U0001F1E0-\U0001F1FF]', text_norm) or
                    re.search(r'[\u200b-\u200f\u202a-\u202e\u2060-\u2064]', text_norm))  # Zero-width characters
    
    # Traditional privacy patterns (fallback)
    if m(PRIVACY_PATTERNS):   return "privacy"
...
def detect_locale(prompt: str) -> str:
    ...
    en_keywords = ["can", "cannot", "capabilities", "safety", "mechanisms", "system", "AI",
                   "what", "how", "why", "when", "where", "role-play", "pretend", "as dan", "jailbreak",
                   "ignore", "previous", "instructions", "override", "bypass", "reveal", "system prompt"]
    has_en_keywords = any(keyword in prompt.lower() for keyword in en_keywords)
    
    # Check for Vietnamese characters
    vi_chars = sum(1 for c in prompt if '\u1e00' <= c <= '\u1eff' or '\u1f00' <= c <= '\u1fff' or 
                   '\u0100' <= c <= '\u017f' or '\u0180' <= c <= '\u024f')
...
def apply_policies(prompt: str):
    ...
    if cat in ("extremism","violence","illegal","privacy","injection","canary","jailbreak"):
...
def safe_reply(category: str, locale: str = "vi", context: Optional[Dict[str, Any]] = None) -> str:
