...
class EmailNotifier:
    ...
    async def send_alert(self, alert: Alert) -> bool:
        ...
        try:
            ...
            msg["Subject"] = (
                f"{emoji} StillMe AI - {alert.severity.upper()}: {alert.title}"
            )
...
This is an automated alert. Please do not reply to this email.
    ...
    def _create_context_section(self, alert: Alert) -> str:
        ...
        for key, value in alert.context.items():
            if key != "metrics":
                context_html += (
                    f'<strong>{key.replace("_", " ").title()}:</strong> {value}<br>'
                )
...
class TelegramNotifier:
    ...
    def _load_config(self):
        ...
        if not all([self.bot_token, self.chat_id]):
            self.logger.warning(
                "Telegram configuration incomplete. Telegram alerts disabled."
            )
            self.config["enabled"] = False
...
class SMSNotifier:
    ...
    async def send_alert(self, alert: Alert) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"SMS alert sent: {alert.alert_id} (SID: {message_obj.sid})"
            )
            return True
...
class AlertManager:
    ...
    def _check_rate_limit(self, alert: Alert) -> bool:
        ...
        # Clean old entries
        while (
            self.rate_limits[alert.alert_type]
            and self.rate_limits[alert.alert_type][0] < hour_ago
        ):
    ...
    def _check_cooldown(self, alert: Alert) -> bool:
        ...
        if cooldown_key in self.cooldowns:
            template = self.templates.get(alert.alert_type)
            cooldown_duration = (
                template.cooldown if template else 600
            )  # 10 minutes default
    ...
    def _update_statistics(
        self, alert: Alert, successful_channels: list[str], failed_channels: list[str]
    ):
...
async def send_alert(
    ...
    return await manager.send_alert(
        alert_type, severity, title, message, component, context, channels
    )
