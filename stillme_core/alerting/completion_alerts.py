...
class CompletionAlertService:
    ...
    def send_learning_completed_alert(self, proposal: Dict[str, Any]):
        ...
        try:
            ...
            message = self._create_completion_message(
                title, objectives, duration, progress
            )

            # Send through all channels
            self.alerting_system.send_alert(
                "🎉 STILLME ĐÃ HOÀN THÀNH HỌC TẬP", message, "success"
            )
    ...
    def _create_completion_message(
        self, title: str, objectives: list, duration: int, progress: float
    ) -> str:
...
⏰ Hoàn thành lúc: {datetime.now().strftime("%H:%M:%S %d/%m/%Y")}
    ...
    def send_batch_completion_alert(self, completed_sessions: list):
        ...
        try:
            ...
            total_duration = sum(
                s.get("estimated_duration", 0) for s in completed_sessions
            )
...
🧠 **StillMe IPC đang trở nên thông minh hơn!**
            ...
            self.alerting_system.send_alert(
                f"🎉 Hoàn thành {total_sessions} bài học", message, "success"
            )
...
📈 **Hệ thống tự động sẽ tiếp tục tìm kiếm kiến thức mới...**
            ...
            self.alerting_system.send_alert(
                "📊 Tổng kết học tập hôm nay", message, "info"
            )

            logger.info("📢 Đã gửi daily summary alert")
