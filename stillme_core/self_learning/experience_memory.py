...
class ExperienceMemory:
    ...
    def _initialize_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_experiences_category ON experiences(category)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_experiences_type ON experiences(experience_type)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_experiences_success ON experiences(success)')
    ...
    def _find_related_experiences(self, context: Dict[str, Any], 
                                action: str, tags: List[str]) -> List[str]:
    ...
    def _calculate_similarity(self, context1: Dict[str, Any], action1: str, 
                            tags1: List[str], exp2: Experience) -> float:
    ...
    def _update_learning_stats(self, experience: Experience):
        ...
        if len(self.learning_stats["recent_activity"]) > 1:
            time_span = (self.learning_stats["recent_activity"][-1]["timestamp"] - 
                        self.learning_stats["recent_activity"][0]["timestamp"])
            if time_span > 0:
                self.learning_stats["learning_velocity"] = len(self.learning_stats["recent_activity"]) / (time_span / 3600)
    ...
    def _learn_from_experience(self, experience: Experience):
        ...
        if len(similar_experiences) >= 3:  # Need at least 3 similar experiences to form a pattern
    ...
    def _extract_pattern(self, experience: Experience, 
                        similar_experiences: List[Experience]) -> Optional[LearningPattern]:
        """Extract a learning pattern from similar experiences"""
        if not similar_experiences:
            return None
        
        # Analyze common conditions
        common_conditions = self._find_common_conditions(experience, similar_experiences)
    ...
    def _find_common_conditions(self, experience: Experience, 
                              similar_experiences: List[Experience]) -> Dict[str, Any]:
        ...
        for key in all_keys:
            ...
            if len(values) >= len(all_contexts) * 0.7:  # 70% of experiences have this key
                # Find most common value
                value_counts = defaultdict(int)
                for value in values:
                    value_counts[str(value)] += 1
                
                most_common_value = max(value_counts.items(), key=lambda x: x[1])[0] if value_counts else ""
            ...
            if values:
                ...
                if non_none_values:
                    if all(isinstance(v, (int, float)) for v in non_none_values):
                        expected_outcome[key] = sum(non_none_values) / len(non_none_values)
                    else:
                        value_counts = defaultdict(int)
                        for value in non_none_values:
                            value_counts[str(value)] += 1
                        expected_outcome[key] = max(value_counts.items(), key=lambda x: x[1])[0] if value_counts else ""
    ...
    def _merge_patterns(self, existing: LearningPattern, new: LearningPattern) -> LearningPattern:
        ...
        total_success = existing.success_rate * existing.frequency + new.success_rate * new.frequency
    ...
    def query_experiences(self, query: ExperienceQuery) -> List[Experience]:
        ...
        for experience in self.experiences:
            ...
            if query.time_range:
                if not (query.time_range[0] <= experience.timestamp <= query.time_range[1]):
                    continue
    ...
    def get_recommendations(self, context: Dict[str, Any], 
                          action: str, tags: List[str]) -> List[Dict[str, Any]]:
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        stats["pattern_accuracy"] = sum(p.success_rate for p in self.patterns) / len(self.patterns) if self.patterns else 0.0
    ...
    def cleanup_old_data(self, days_to_keep: int = 90):
        ...
        self.experiences = [exp for exp in self.experiences if exp.timestamp > cutoff_time]
        removed_count = old_count - len(self.experiences)
        
        # Remove old patterns with low frequency
        old_pattern_count = len(self.patterns)
        self.patterns = [p for p in self.patterns if p.frequency > 2 or p.last_updated > cutoff_time]
        removed_pattern_count = old_pattern_count - len(self.patterns)
        
        logger.info(f"Cleaned up {removed_count} old experiences and {removed_pattern_count} old patterns")
    ...
    def _cleanup_database(self, cutoff_time: float):
        ...
        try:
            ...
            # Remove old experiences
            cursor.execute('DELETE FROM experiences WHERE timestamp < ?', (cutoff_time,))
            
            # Remove low-frequency patterns
            cursor.execute('DELETE FROM patterns WHERE frequency <= 2 AND last_updated < ?', (cutoff_time,))
