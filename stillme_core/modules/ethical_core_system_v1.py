...
if not ethical_logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - User ID: %(user_id)s - Violation Type: %(violation_type)s - Severity: %(severity)s - Message: %(message)s"
    )
...
def close_violation_logger():
    ...
    for handler in ethical_logger.handlers[
        :
    ]:  # Lặp trên một bản sao của danh sách handler
        handler.close()
...
class OpenRouterClient:
    def __init__(
        self, api_key: str, model: OpenRouterModel = OpenRouterModel.MISTRAL_7B_INSTRUCT
    ):
        if not api_key:
    ...
    async def chat_completion(
        self, messages: list, temperature: float = 0.7, max_tokens: int = 150
    ) -> str:
        headers = {
...
class EthicsGuard:
    def __init__(
        ...
        rules_file: str = "config/ethical_rules.json",
    ):
        ...
        try:
            with open(self.rules_file, encoding="utf-8") as f:
                ...
                self.forbidden_keywords = [
                    k.lower() for k in rules.get("forbidden_keywords", [])
                ]
                self.sensitive_topics = [
                    t.lower() for t in rules.get("sensitive_topics", [])
                ]
            ...
            ethical_logger.error(
                "Error decoding JSON from ethical rules file. Using empty rules."
            )
            self.forbidden_keywords = []
            self.sensitive_topics = []
        except Exception as e:
            ethical_logger.error(
                f"An unexpected error occurred loading ethical rules: {e}"
            )
            self.forbidden_keywords = []
            self.sensitive_topics = []
        ...
        try:
            ...
            if safety_assessment.get("is_toxic"):
                return (
                    ...
                    Severity.MEDIUM,
                    safety_assessment.get("reason", "Nội dung độc hại."),
                )
            if safety_assessment.get("is_hate_speech"):
                return (
                    ...
                    Severity.HIGH,
                    safety_assessment.get("reason", "Nội dung thù địch."),
                )
            ...
            for topic in detected_sensitive:
                if topic.lower() in self.sensitive_topics:
                    return (
                        ...
                        Severity.HIGH,
                        f"Chủ đề nhạy cảm được phát hiện: {topic}.",
                    )
            ...
            return (
                False,
                ViolationType.LLM_ERROR,
                Severity.CRITICAL,
                f"Đã xảy ra lỗi trong quá trình kiểm tra an toàn input: {e}",
            )
        ...
        try:
            ...
            if safety_assessment.get("is_toxic"):
                return (
                    ...
                    ViolationType.TOXIC_CONTENT,
                    Severity.MEDIUM,
                    safety_assessment.get("reason", "Nội dung độc hại trong phản hồi."),
                )
            if safety_assessment.get("is_hate_speech"):
                return (
                    False,
                    ...
                    safety_assessment.get(
                        "reason", "Nội dung thù địch trong phản hồi."
                    ),
                )
            if not safety_assessment.get("is_appropriate"):
                return (
                    False,
                    ViolationType.ETHICAL_NON_COMPLIANCE,
                    Severity.MEDIUM,
                    safety_assessment.get("reason", "Phản hồi không phù hợp."),
                )
    ...
    async def assess_vulnerability(self, user_input: str) -> tuple[bool, str]:
        try:
            ...
            return vulnerability_assessment.get(
                "is_vulnerable", False
            ), vulnerability_assessment.get("reason", "Không có dấu hiệu.")
        except Exception as e:
...
class ConscienceCore:
    ...
    async def evaluate_ethical_compliance(
        ...
        try:
            ...
            return (
                compliance_assessment.get("is_compliant", False),
                compliance_assessment.get("compliance_score", 0.0),
                compliance_assessment.get("reason", "Không rõ."),
            )
        except Exception as e:
            ...
            return (
                False,
                0.0,
                f"Đã xảy ra lỗi trong quá trình đánh giá tuân thủ đạo đức: {e}",
            )

    async def adjust_response_ethically(
        self, user_input: str, ai_response: str, violation_reason: str
    ) -> str:
        try:
...
class SelfCritic:
    ...
    async def analyze_and_learn(
        ...
        try:
            ...
            return {
                "root_cause": f"Lỗi phân tích: {e}",
                "suggested_action": "Không thể phân tích nguyên nhân gốc rễ.",
            }
...
class EthicalCoreSystem:
    def __init__(self):
        ...
        self.logger = get_logger(
            "StillMe.EthicalCore", log_file="logs/ethical_core.log", json_format=True
        )
        ...
        try:
            ...
            if not is_input_safe:
                ...
                if input_violation_type is not None and input_severity is not None:
                    self.self_critic.log_ethical_violation(
                        user_id,
                        user_input,
                    ...
                    await self.self_critic.analyze_and_learn(
                        user_id,
                        user_input,
                        original_ai_response,
                        input_violation_type,
                ...
                return (
                    self.ethics_guard.violation_response,
                    is_compliant,
                    violation_message,
                )
            ...
            if not is_output_safe:
                ...
                adjusted_response = (
                    await self.conscience_core.adjust_response_ethically(
                        user_input, original_ai_response, output_reason
                    )
                )
                ...
                if not is_adjusted_compliant:
                    ...
                    self.self_critic.log_ethical_violation(
                        ...
                        ViolationType.ETHICAL_NON_COMPLIANCE,
                        Severity.MEDIUM,
                        adjusted_reason + " (sau điều chỉnh)",
                    )
                    await self.self_critic.analyze_and_learn(
                        user_id,
                        user_input,
                    ...
                    violation_message += (
                        "Phản hồi đã được điều chỉnh để tuân thủ đạo đức."
                    )
                    if (
                        output_violation_type is not None
                        and output_severity is not None
                    ):
                        await self.self_critic.analyze_and_learn(
                            user_id,
                            user_input,
            ...
            ethical_logger.error(
                f"An unexpected error occurred during interaction processing: {e}",
