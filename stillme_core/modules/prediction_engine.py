...
class PredictionEngine:
    ...
    def _analyze_technology_trends(
        ...
        reasoning = self._generate_reasoning(
            tech_name, signals, avg_potential, direction
        )
    ...
    def _generate_reasoning(
        ...
        if potential_score > 70:
            reasoning_parts.append(
                f"{tech_name} shows strong potential with a score of {potential_score:.1f}"
            )
        elif potential_score > 50:
            reasoning_parts.append(
                f"{tech_name} shows moderate potential with a score of {potential_score:.1f}"
            )
        else:
            reasoning_parts.append(
                f"{tech_name} shows limited potential with a score of {potential_score:.1f}"
            )
        ...
        if len(signal_sources) > 1:
            reasoning_parts.append(
                f"supported by data from {', '.join(set(signal_sources))}"
            )
        ...
        if strong_signals:
            ...
            reasoning_parts.append(
                f"with particularly strong signals from {', '.join(set(strong_sources))}"
            )
    ...
    def generate_business_recommendations(
        ...
        for prediction in predictions:
            ...
            ):  # Only recommend for confident predictions
    ...
    def _create_recommendations_for_trend(
        ...
        if (
            prediction.potential_score > 70
            and prediction.direction == TrendDirection.RISING
        ):
            ...
            recommendations.append(
                BusinessRecommendation(
                    ...
                    supporting_data=[
                        f"Potential score: {prediction.potential_score:.1f}",
            ...
            recommendations.append(
                BusinessRecommendation(
                    ...
                    confidence=prediction.confidence_score,
                    supporting_data=[
                        f"Current potential: {prediction.potential_score:.1f}"
                    ],
                )
            )
