...
class ComplexityAnalyzer:
    ...
    def __init__(self):
        ...
        self.academic_terms: set[str] = {
            ...
            "bất toàn", "gödel", "turing", "einstein", "newton", "darwin",
        ...
        self.weights = {
            ...
            'complex_indicators': float(os.getenv('COMPLEXITY_WEIGHT_INDICATORS', '0.25')),
...
class UnifiedAPIManager:
    ...
    def choose_model(self, prompt: str, debug: bool = False) -> str:
        ...
        if prompt_len > 3000:
            if "deepseek-coder:6.7b" in self.model_preferences:
                self.logger.info(f"🎯 Selected deepseek-coder:6.7b (long prompt: {prompt_len} chars)")
                return "deepseek-coder:6.7b"
            elif "gemma2:2b" in self.model_preferences:
                self.logger.info(f"🎯 Selected gemma2:2b (long prompt: {prompt_len} chars)")
                return "gemma2:2b"
        ...
        # Low complexity → use simple local model
            ...
            if "gemma2:2b" in self.model_preferences:
                self.logger.info(f"🎯 Selected gemma2:2b (low complexity: {complexity_score:.3f})")
                return "gemma2:2b"
    ...
    def translate(self, text: str, src_lang: str, tgt_lang: str, quality_hint: Optional[str] = None) -> dict:
        """
    ...
    def call_deepseek_api(self, prompt: str) -> str:
        ...
        try:
            ...
            try:
                ...
                self.logger.error("❌ requests not available. Install with: pip install requests")
