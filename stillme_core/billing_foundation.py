...
class BillingFoundation:
    ...
    """
    def __init__(self, db_path: str = "billing_data.db", config: Dict[str, Any] = DEFAULT_CONFIG):
    ...
    async def generate_invoice(self, data: Dict[str, Any]) -> Invoice:
        ...
        try:
            ...
            cursor.execute("INSERT INTO invoices VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                           (invoice.invoice_id, invoice.customer_id, json.dumps(invoice.items),
                            invoice.subtotal, invoice.taxes, invoice.total, invoice.status, invoice.due_date))
            conn.commit()
            conn.close()
        ...
        try:
            ...
            cursor.execute("INSERT INTO payments VALUES (?, ?, ?, ?, ?, ?, ?)",
                           (payment.payment_id, payment.invoice_id, payment.amount,
                            payment.method, payment.status, payment.timestamp, payment.transaction_id))
            
            # Cập nhật trạng thái hóa đơn
            if status == "completed":
                cursor.execute("UPDATE invoices SET status = 'paid' WHERE invoice_id = ?", (payment.invoice_id,))
            
            conn.commit()
            conn.close()
            
            logger.info(f"Đã xử lý thanh toán {payment.payment_id} với trạng thái: {status}")
            return payment
        except Exception as e:
    ...
    async def track_billing(self, data: Dict[str, Any]) -> BillingRecord:
        ...
        try:
            logger.info(f"Đang ghi lại bản ghi billing cho khách hàng: {data['customer_id']}")
            ...
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO billing_records VALUES (?, ?, ?, ?, ?, ?, ?)",
                           (record.record_id, record.customer_id, record.service_type,
                            record.usage, record.amount, record.billing_cycle, record.status))
            conn.commit()
            conn.close()
    ...
    async def calculate_taxes(self, subtotal: float, jurisdiction: str) -> TaxCalculation:
        ...
        try:
            ...
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO tax_calculations VALUES (NULL, ?, ?, ?, ?, ?)",
                           ("VAT", tax_calc.rate, tax_calc.amount, tax_calc.jurisdiction, tax_calc.exemption_status))
            conn.commit()
            conn.close()
    ...
    async def recognize_revenue(self, data: Dict[str, Any]) -> RevenueRecognition:
        ...
        try:
            logger.info(f"Đang nhận diện doanh thu cho giai đoạn: {data['service_period']}")
            ...
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO revenue_recognition VALUES (?, ?, ?, ?, ?)",
                           (revenue.revenue_id, revenue.amount, revenue.recognition_date,
                            revenue.service_period, revenue.status))
            conn.commit()
            conn.close()
