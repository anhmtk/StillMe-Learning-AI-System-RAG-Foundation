...
class IntegrationTesting:
    ...
    def __init__(self, 
        ...
        logger.info("✅ IntegrationTesting initialized với enterprise-grade configuration")
    ...
    def run_system_integration_tests(self) -> SystemIntegrationReport:
        ...
        try:
            ...
            overall_performance_score = self._calculate_overall_performance_score(test_results)
            ...
            logger.info(f"🧪 System integration tests completed: {passed_tests}/{total_tests} passed in {(time.time() - start_time) * 1000:.1f}ms")
            ...
            return SystemIntegrationReport(
                ...
                recommendations=[f"Integration testing failed: {str(e)}"],
                timestamp=datetime.now()
            )
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                return IntegrationTestResult(
                    ...
                    error_message=None if status == "passed" else "Database connection failed",
                    timestamp=datetime.now()
                )
    ...
    def _test_database_performance(self) -> IntegrationTestResult:
        ...
        try:
            ...
            status = "passed" if avg_query_time < self.config['performance_threshold_ms'] else "failed"
            duration_ms = (time.time() - start_time) * 1000
            ...
            return IntegrationTestResult(
                ...
                error_message=None if status == "passed" else f"Average query time {avg_query_time:.1f}ms exceeds threshold",
                timestamp=datetime.now()
            )
        ...
        try:
            with sqlite3.connect(self.metrics_db_path) as conn:
                ...
                return IntegrationTestResult(
                    ...
                    error_message=None if status == "passed" else f"Found {integrity_issues} data integrity issues",
                    timestamp=datetime.now()
                )
    ...
    def _test_memory_usage_integration(self) -> IntegrationTestResult:
        ...
        try:
            ...
            return IntegrationTestResult(
                ...
                error_message=None if status == "passed" else f"Memory usage {memory_usage:.1f}MB exceeds 4GB limit",
                timestamp=datetime.now()
            )
    ...
    def _test_response_time_integration(self) -> IntegrationTestResult:
        ...
        try:
            ...
            status = "passed" if response_time < self.config['performance_threshold_ms'] else "failed"
            ...
            return IntegrationTestResult(
                ...
                error_message=None if status == "passed" else f"Response time {response_time:.1f}ms exceeds threshold",
                timestamp=datetime.now()
            )
    ...
    def _calculate_overall_performance_score(self, test_results: List[IntegrationTestResult]) -> float:
        ...
        try:
            ...
            for test in failed_tests:
                if "database" in test.test_type:
                    recommendations.append("Review database configuration and performance")
            ...
            if not recommendations:
                recommendations.append("All integration tests passed - system is healthy")
    ...
    def get_integration_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                'integration_test_types': ['database_integration', 'module_integration', 'performance_integration', 'scalability'],
...
# Factory function
def create_integration_testing(metrics_db_path: str, config: Optional[Dict[str, Any]] = None) -> IntegrationTesting:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-tests", action="store_true", help="Run system integration tests")
    parser.add_argument("--summary", action="store_true", help="Get integration summary")
