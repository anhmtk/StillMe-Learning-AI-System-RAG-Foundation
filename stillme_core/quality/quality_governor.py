...
class QualityGovernor:
    ...
    def assess_code_quality(self) -> QualityReport:
        ...
        for metric in QualityMetric:
            ...
            metric_violations = self._check_metric_violations(metric, metric_data, file_metrics)
            violations.extend(metric_violations)
        
        # Calculate overall quality score
        overall_score = self._calculate_overall_quality_score(metrics_summary, violations)
        
        # Generate recommendations
        recommendations = self._generate_quality_recommendations(metrics_summary, violations)
        ...
        return QualityReport(
            ...
            quality_trends={k: v[-1] if v else 0.0 for k, v in self.quality_history.items()},
    ...
    def _calculate_file_metrics(self, file_path: Path) -> Dict[QualityMetric, float]:
        ...
        try:
            ...
            metrics[QualityMetric.CYCLOMATIC_COMPLEXITY] = self._calculate_cyclomatic_complexity(tree)
            
            # Calculate cognitive complexity
            metrics[QualityMetric.COGNITIVE_COMPLEXITY] = self._calculate_cognitive_complexity(tree)
            
            # Calculate documentation coverage
            metrics[QualityMetric.DOCUMENTATION_COVERAGE] = self._calculate_documentation_coverage(content)
            
            # Calculate maintainability index
            metrics[QualityMetric.MAINTAINABILITY_INDEX] = self._calculate_maintainability_index(
            ...
            metrics[QualityMetric.CODE_SMELLS] = self._calculate_code_smells(tree, content)
    ...
    def _calculate_maintainability_index(self, cyclomatic_complexity: float, 
                                       lines_of_code: int, doc_coverage: float) -> float:
    ...
    def _aggregate_metric(self, metric: QualityMetric, file_metrics: List[Dict[QualityMetric, float]]) -> Dict[str, Any]:
    ...
    def _check_metric_violations(self, metric: QualityMetric, metric_data: Dict[str, Any], 
                               file_metrics: List[Dict[QualityMetric, float]]) -> List[QualityViolation]:
        ...
        if current_standard in [QualityStandard.POOR, QualityStandard.CRITICAL]:
            violation = QualityViolation(
                ...
                severity="high" if current_standard == QualityStandard.CRITICAL else "medium",
    ...
    def _determine_standard(self, value: float, thresholds: Dict[str, float]) -> QualityStandard:
    ...
    def _calculate_overall_quality_score(self, metrics_summary: Dict[QualityMetric, Dict[str, Any]], 
                                       violations: List[QualityViolation]) -> float:
        ...
        final_score = (total_score / total_weight) - violation_penalty if total_weight > 0 else 0
    ...
    def _normalize_metric_score(self, metric: QualityMetric, value: float) -> float:
        ...
        if metric in self.quality_thresholds:
            ...
            if metric in [QualityMetric.CYCLOMATIC_COMPLEXITY, QualityMetric.COGNITIVE_COMPLEXITY, 
                         QualityMetric.CODE_DUPLICATION, QualityMetric.CODE_SMELLS]:
    ...
    def _generate_quality_recommendations(self, metrics_summary: Dict[QualityMetric, Dict[str, Any]], 
                                        violations: List[QualityViolation]) -> List[str]:
        ...
        if overall_score >= 90:
            ...
            recommendations.append("👍 Good code quality. Address remaining issues for excellence.")
        elif overall_score >= 70:
            recommendations.append("📋 Acceptable code quality. Focus on improving key metrics.")
        elif overall_score >= 60:
            recommendations.append("⚠️ Code quality needs improvement. Prioritize critical issues.")
        else:
        ...
        if critical_violations:
            recommendations.append(f"🚨 {len(critical_violations)} critical quality violations need immediate attention.")
        
        return recommendations
    
    def _update_quality_trends(self, metrics_summary: Dict[QualityMetric, Dict[str, Any]], 
                             overall_score: float):
        ...
        if len(self.quality_history["overall_score"]) > 30:
            self.quality_history["overall_score"] = self.quality_history["overall_score"][-30:]
        ...
        for metric, data in metrics_summary.items():
            ...
            if len(self.quality_history[metric_key]) > 30:
                self.quality_history[metric_key] = self.quality_history[metric_key][-30:]
