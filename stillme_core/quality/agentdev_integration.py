...
class AgentDevQualityIntegration:
    ...
    def run_quality_gates(self,
        ...
        try:
            ...
            for result in results:
                ...
                self.logger.info(f"{status_icon} Quality gate: {result.gate_type.value} = {result.score} ({result.status})")
    ...
    def _run_performance_gate(self, performance_metrics: Dict[str, Any] = None) -> Optional[QualityGateResult]:
        ...
        try:
            ...
            performance_score = max(0, 10 - (response_time/1000 + memory_usage/100 + cpu_usage/100))
    ...
    def _run_maintainability_gate(self, code_content: str) -> Optional[QualityGateResult]:
        ...
        try:
            ...
            function_definitions = sum(1 for line in lines if line.strip().startswith('def '))
            class_definitions = sum(1 for line in lines if line.strip().startswith('class '))

            # Calculate maintainability score
            docstring_ratio = docstring_lines / max(1, function_definitions + class_definitions)
    ...
    def _run_ethics_gate(self, code_content: str) -> Optional[QualityGateResult]:
        ...
        try:
            ...
            privacy_count = sum(1 for keyword in privacy_keywords if keyword in content_lower)

            # Calculate ethics score (10 - violations)
            ethics_score = max(0, 10 - (harmful_count * 2 + bias_count * 1 + privacy_count * 2))
    ...
    def update_gate_threshold(self, gate_type: QualityGate, threshold: float):
        ...
        try:
            ...
            self.logger.info(f"📝 Gate threshold updated: {gate_type.value} = {threshold}")
