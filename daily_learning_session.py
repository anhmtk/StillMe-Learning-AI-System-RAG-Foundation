...
def run_daily_learning_session():
    ...
    try:
        ...
        memory_manager = (
            framework.layered_memory if hasattr(framework, "layered_memory") else None
        )

        # Get SelfImprovementManager from framework
        improvement_manager = (
            framework.self_improvement_manager
            if hasattr(framework, "self_improvement_manager")
            else None
        )
        ...
        if improvement_result["status"] == "success":
            ...
            print(
                f"📈 Suggestions generated: {improvement_result['suggestions_count']}"
            )
            print(
                f"📤 Suggestions submitted: {improvement_result['suggestions_submitted']}"
            )
            ...
            if perf_analysis:
                ...
                print(
                    f"   Score trend: {perf_analysis.get('overall_score_trend', 'stable')}"
                )
                if perf_analysis.get("weak_categories"):
                    print(
                        f"   Weak categories: {', '.join(perf_analysis['weak_categories'])}"
                    )
                if perf_analysis.get("strong_categories"):
                    print(
                        f"   Strong categories: {', '.join(perf_analysis['strong_categories'])}"
                    )
        else:
            print(
                f"⚠️ Learning improvement analysis failed: {improvement_result.get('error', 'Unknown error')}"
            )
...
def add_custom_learning_case():
    ...
    try:
        ...
        # Input từ user
        category = input(
            "Category (programming/ai_ml/system_design/debugging/creative): "
        ).strip()
