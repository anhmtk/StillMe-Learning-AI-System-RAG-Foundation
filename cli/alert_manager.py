...
class AlertCLI:
    ...
    async def send_learning_test_alert(self, args):
        ...
        alerts_sent = await self.learning_alert_manager.check_learning_session_alerts(
            test_metrics
        )
...
Examples:
  ...
  python -m cli.alert_manager history --limit 20
    ...
    test_parser.add_argument(
        "--channels",
        type=str,
        help="Comma-separated list of channels (email,desktop,telegram,sms,webhook)",
    )
    test_parser.add_argument(
        "--severity",
    ...
    test_resource_parser = subparsers.add_parser(
        "test-resource", help="Send test resource alert"
    )
    test_resource_parser.add_argument(
        "--resource", choices=["memory", "cpu", "disk", "network"], help="Resource type"
    )
    ...
    test_channel_parser = subparsers.add_parser(
        "test-channel", help="Test individual notification channel"
    )
    test_channel_parser.add_argument(
        "--channel",
        choices=["email", "desktop", "telegram", "sms", "webhook"],
        required=True,
    ...
    history_parser.add_argument(
        "--limit", type=int, help="Number of alerts to show (default: 10)"
    )
