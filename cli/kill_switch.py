...
def format_status(status: dict) -> str:
    ...
    state_emoji = {"disarmed": "✅", "armed": "⚠️", "fired": "🚨"}
...
Examples:
  ...
  %(prog)s --audit --limit 20          # Show last 20 audit entries
    ...
    action_group.add_argument(
        "--status", action="store_true", help="Show kill switch status"
    )
    action_group.add_argument("--arm", action="store_true", help="Arm the kill switch")
    action_group.add_argument(
        "--fire", action="store_true", help="Fire the kill switch (emergency stop)"
    )
    action_group.add_argument(
        "--disarm", action="store_true", help="Disarm the kill switch"
    )
    action_group.add_argument("--audit", action="store_true", help="Show audit log")
    ...
    parser.add_argument("--reason", help="Reason for the action")
    parser.add_argument(
        "--limit", type=int, default=100, help="Limit audit log entries (default: 100)"
    )
        ...
        if args.json:
            ...
            if args.status:
                ...
                if result:
                    ...
                    print(
                        f"{success_emoji} {result.get('message', 'Action completed')}"
                    )
