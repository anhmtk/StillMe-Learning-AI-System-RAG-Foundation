...
Examples:
  ...
  python -m stillme_core.agent_dev --goal "Add new feature" --max-steps 3 --repo-root /path/to/repo
    ...
    try:
        result = run_agent(
            goal=args.goal, max_steps=args.max_steps, repo_root=args.repo_root
        )
        ...
        if result["steps"]:
            ...
            print("=" * 50)
            print(
                f"{'ID':<3} {'Action':<15} {'Status':<8} {'Duration':<10} {'Description'}"
            )
            print("-" * 50)
            ...
            for step in result["steps"]:
                ...
                print(
                    f"{step['id']:<3} {step['action']:<15} {status:<8} {duration_str:<10} {description}"
                )
