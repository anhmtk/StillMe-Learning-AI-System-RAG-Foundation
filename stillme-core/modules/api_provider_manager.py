...
class UnifiedAPIManager:
    ...
    def choose_model(self, prompt: str) -> str:
        ...
        if (
            any(keyword in prompt.lower() for keyword in creative_keywords)
            and "gpt-4o" in self.model_preferences
        ):
            return "gpt-4o"
    ...
    def call_deepseek_api(self, prompt: str) -> str:
        ...
        try:
            ...
            response = asyncio.run(
                self.http_client.post(url, headers=headers, json_data=payload)
            )
    ...
    def call_ollama_api(self, prompt: str) -> str:
        ...
        try:
            ...
            self.logger.info(
                f"Ollama: Calling model gemma2:2b with prompt: {prompt[:50]}..."
            )
            ...
            response = asyncio.run(
                self.http_client.post(
                    "http://localhost:11434/api/chat", json_data=payload, timeout=60
                )
            )
