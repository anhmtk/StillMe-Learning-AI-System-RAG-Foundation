...
class AutomatedScheduler:
    ...
    async def _initialize_learning_manager(self):
        ...
        try:
            ...
            improvement_manager = getattr(
                self.framework, "self_improvement_manager", None
            )

            self.learning_manager = DailyLearningManager(
                memory_manager=memory_manager, improvement_manager=improvement_manager
            )

            self.logger.info(
                "✅ Learning manager initialized with framework integration"
            )
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(
                f"🔧 Monthly improvement scheduled: {self.config.monthly_improvement_day} at {self.config.monthly_improvement_time}"
            )
    ...
    async def _run_weekly_analysis(self):
        ...
        try:
            ...
            if not self.learning_manager:
                self.logger.warning(
                    "⚠️ Learning manager not available, skipping analysis"
                )
                return
            ...
            self.logger.info(
                f"🔍 Performance trend: {analysis.get('performance_analysis', {}).get('overall_score_trend', 'unknown')}"
            )
    ...
    async def _run_monthly_improvement(self):
        ...
        try:
            ...
            if not self.learning_manager:
                self.logger.warning(
                    "⚠️ Learning manager not available, skipping improvement"
                )
                return
            ...
            self.logger.info(
                f"📤 Suggestions submitted: {improvement_result.get('suggestions_submitted', False)}"
            )

            # Log improvement suggestions
            for i, suggestion in enumerate(suggestions, 1):
                self.logger.info(
                    f"💡 Suggestion {i}: {suggestion.get('description', 'N/A')}"
                )
    ...
    async def _run_health_check(self):
        ...
        try:
            ...
            self.logger.info(
                f"💚 Health check - Jobs: {active_jobs}, Learning: {learning_health}, Framework: {framework_health}"
            )
