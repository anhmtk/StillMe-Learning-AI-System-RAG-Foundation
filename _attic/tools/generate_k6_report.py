...
def analyze_k6_metrics(summary, results):
    ...
    if results:
        durations = [
            r.get("data", {}).get("http_req_duration", 0)
            for r in results
            if "data" in r
        ]
        status_codes = [
            r.get("data", {}).get("http_req_status", 0) for r in results if "data" in r
        ]
        ...
        if durations:
            metrics["calculated"] = {
                ...
                "error_rate": sum(1 for s in status_codes if s >= 400)
                / len(status_codes)
                if status_codes
                else 0,
            }
...
def create_latency_chart(results, output_dir):
    ...
    durations = [
        r.get("data", {}).get("http_req_duration", 0) for r in results if "data" in r
    ]
    if not durations:
        return None
    ...
    plt.axvline(
        np.percentile(durations, 50), color="green", linestyle="--", label="p50"
    )
    plt.axvline(
        np.percentile(durations, 95), color="orange", linestyle="--", label="p95"
    )
...
def create_rps_chart(results, output_dir):
    ...
    while current_time < end_time:
        ...
        window_requests = [
            r for r in results if current_time <= r.get("time", 0) < window_end
        ]
    ...
    while current_time < end_time:
        ...
        if window_requests:
            errors = sum(
                ...
                if r.get("data", {}).get("http_req_status", 200) >= 400
            )
...
def create_resource_chart(resource_df, output_dir):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(
        resource_df["datetime"], resource_df["cpu_percent"], linewidth=2, color="blue"
    )
    ...
    plt.subplot(2, 1, 2)
    plt.plot(
        resource_df["datetime"], resource_df["mem_percent"], linewidth=2, color="green"
    )
...
def generate_markdown_report(metrics, charts, output_dir):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"| P95 Latency | < 500ms | {p95_latency:.2f}ms | {'✅ PASS' if p95_pass else '❌ FAIL'} |\n"
        )
        f.write(
            f"| P99 Latency | < 1000ms | {p99_latency:.2f}ms | {'✅ PASS' if p99_pass else '❌ FAIL'} |\n"
        )
        f.write(
            f"| Error Rate | < 1% | {error_rate:.2f}% | {'✅ PASS' if error_pass else '❌ FAIL'} |\n"
        )
        f.write(
            f"| Throughput | ≥ 200 RPS | {rps:.2f} RPS | {'✅ PASS' if rps_pass else '❌ FAIL'} |\n"
        )
        ...
        f.write(
            f"- **Total Requests**: {metrics.get('http_reqs', {}).get('count', 0):,}\n"
        )
        f.write(f"- **Average RPS**: {rps:.2f}\n")
        f.write(
            f"- **P50 Latency**: {metrics.get('http_req_duration', {}).get('p50', 0):.2f}ms\n"
        )
        ...
        if not overall_pass:
            ...
            if not p99_pass:
                f.write(
                    f"- **P99 Latency**: {p99_latency:.2f}ms exceeds 1000ms threshold\n"
                )
            if not error_pass:
        ...
        f.write(
            f"**Status**: {'✅ COMPLIANT' if overall_pass else '❌ NON-COMPLIANT'}\n\n"
        )
...
def main():
    ...
    if metrics:
        ...
        print(
            f"   Overall: {'✅ SEAL-GRADE PASSED' if overall_pass else '❌ SEAL-GRADE FAILED'}"
        )
