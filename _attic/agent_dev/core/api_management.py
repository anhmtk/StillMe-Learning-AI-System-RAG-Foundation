...
class APIManagementSystem:
    ...
    def _load_existing_apis(self):
        ...
        openapi_files = list(self.api_dir.glob("*.yaml")) + list(
    ...
    def _save_endpoint(self, endpoint: APIEndpoint):
        ...
        endpoint_file = (
            self.api_dir / f"endpoint_{endpoint.method.value}_{safe_path}.json"
        )
    ...
    def create_contract(
        self, endpoint_id: str, expected_schema: dict[str, Any]
    ) -> APIContract:
        """Create API contract"""
    ...
    def _save_contract(self, contract: APIContract):
        ...
        safe_id = (
            contract.contract_id.replace(":", "_").replace("/", "_").replace("\\", "_")
        )
    ...
    async def _test_contract(
        self, endpoint: APIEndpoint, base_url: str
    ) -> dict[str, Any]:
        """Contract test"""
        ...
        for c in self.contracts.values():
            if (
                c.endpoint.path == endpoint.path
                and c.endpoint.method == endpoint.method
            ):
                contract = c
                break
        ...
        async def make_request():
            async with semaphore:
                return await self.test_endpoint(
                    endpoint_id, TestType.PERFORMANCE, base_url
                )
    ...
    def generate_api_report(self) -> APIManagementReport:
        ...
        if self.test_results:
            response_times = [
                r.response_time for r in self.test_results if r.response_time > 0
            ]
            if response_times:
                performance_metrics = {
        ...
        for result in self.test_results:
            if result.status_code in [401, 403, 500]:
                security_issues.append(
                    f"Security issue in {result.endpoint}: {result.error_message}"
                )
        ...
        for result in self.test_results:
            if result.test_type == TestType.CONTRACT and result.status == "failed":
                contract_violations.append(
                    f"Contract violation in {result.endpoint}: {result.error_message}"
                )
