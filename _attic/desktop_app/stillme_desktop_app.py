...
class StillMeDesktopApp:
    ...
    def setup_chat_area(self, parent):
        ...
        self.chat_text.tag_configure(
            "user", foreground="#3B82F6", font=("Arial", 11, "bold")
        )
        self.chat_text.tag_configure(
            "assistant", foreground="#10B981", font=("Arial", 11, "bold")
        )
        self.chat_text.tag_configure(
            "system", foreground="#F59E0B", font=("Arial", 10, "italic")
        )
        self.chat_text.tag_configure(
            "timestamp", foreground="#6B7280", font=("Arial", 9)
        )
        self.chat_text.tag_configure(
            "metadata", foreground="#9CA3AF", font=("Arial", 9)
        )
    ...
    def send_message(self, event=None):
        ...
        threading.Thread(
            target=self._send_to_server, args=(message,), daemon=True
        ).start()
    ...
    def _send_to_server(self, message):
        ...
        try:
            ...
            self.root.after(
                0, lambda: self.add_system_message("🤖 StillMe is typing...")
            )
            ...
            payload = {
                ...
                "metadata": {"persona": "assistant", "language": "vi", "debug": True},
            }
            ...
            self.root.after(
                0, lambda err=e: self.add_system_message(f"❌ Error: {str(err)}")
            )
    ...
    def add_assistant_message(self, data, latency_ms):
        ...
        self.chat_text.insert(
            tk.END, f"  Model: {model} | Latency: {latency_ms}ms\n", "metadata"
        )
    ...
    def update_telemetry_display(self):
        ...
        self.telemetry_text.insert(
            tk.END, f"Messages: {self.session_metrics['total_messages']}\n"
        )
        self.telemetry_text.insert(
            tk.END, f"Tokens: {self.session_metrics['total_tokens']}\n"
        )
        self.telemetry_text.insert(
            tk.END, f"Cost: ${self.session_metrics['total_cost']:.4f}\n"
        )
        self.telemetry_text.insert(
            tk.END, f"Avg Latency: {self.session_metrics['average_latency']:.0f}ms\n"
        )
        self.telemetry_text.insert(
            tk.END, f"Models: {', '.join(self.session_metrics['models_used'])}\n"
        )
        ...
        for telemetry in self.telemetry_data[-5:]:  # Last 5 requests
            ...
            self.telemetry_text.insert(
                tk.END,
                f"  Latency: {latency}ms | Tokens: {tokens} | Cost: ${cost:.4f}\n",
            )
    ...
    def generate_metrics_report(self):
        ...
        report += (
            f"⚡ Average Latency: {self.session_metrics['average_latency']:.0f}ms\n"
        )
        report += (
            f"🤖 Models Used: {', '.join(self.session_metrics['models_used'])}\n\n"
        )
        ...
        if self.telemetry_data:
            ...
            report += (
                f"  P95 Latency: {sorted(latencies)[int(len(latencies)*0.95)]}ms\n\n"
            )
    ...
    def show_founder_console(self):
        ...
        if not self.founder_mode:
            ...
            passcode = simpledialog.askstring(
                "Founder Console", "Enter founder passcode:", show="*"
            )
    ...
    def generate_founder_console_content(self):
        ...
        content += (
            f"  Connection: {'✅ Active' if self.telemetry_data else '❓ Unknown'}\n\n"
        )
    ...
    def show_settings(self):
        ...
        server_frame = tk.LabelFrame(
            settings_frame, text="Server Settings", bg="#1E293B", fg="#F8FAFC"
        )
        server_frame.pack(fill=tk.X, pady=(0, 20))

        tk.Label(server_frame, text="Base URL:", bg="#1E293B", fg="#F8FAFC").pack(
        ...
        features_frame = tk.LabelFrame(
            settings_frame, text="Features", bg="#1E293B", fg="#F8FAFC"
        )
        ...
        tk.Button(
            button_frame,
            text="Save",
            command=lambda: self.save_settings(url_entry.get(), telemetry_var.get()),
            bg="#3B82F6",
            fg="white",
    ...
    def show_quick_actions(self):
        ...
        for action_text, command in actions:
            btn = tk.Button(
                actions_window,
                text=action_text,
                command=lambda cmd=command: self.execute_quick_action(
                    cmd, actions_window
                ),
                bg="#3B82F6",
                fg="white",
        ...
        try:
            ...
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(
                    {
                        "session_id": self.session_id,
                        "session_start": self.session_metrics[
                            "session_start"
                        ].isoformat(),
                        "messages": [
                            {
