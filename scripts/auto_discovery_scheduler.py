...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('auto_discovery.log'),
    ...
    def scheduled_discovery(self):
        ...
        try:
            ...
            if proposals_created > 0:
                logger.info(f"🎯 Hệ thống đã tự động phát hiện {proposals_created} chủ đề học tập mới")
            else:
                logger.info("ℹ️ Không có chủ đề mới phù hợp trong lần discovery này")
                
        except Exception as e:
            logger.error(f"❌ Lỗi trong auto-discovery: {e}")
            logger.error(f"🔧 Sẽ thử lại trong 6 giờ tới")
