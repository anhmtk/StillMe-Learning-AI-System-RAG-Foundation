...
def run_quick_evaluation(api_url: str = "http://localhost:8000", num_questions: int = 10):
    ...
    logger.info("=" * 60)
    citations_found = [r for r in truthfulqa_results.detailed_results if r.has_citation]
    citations_missing = [r for r in truthfulqa_results.detailed_results if not r.has_citation]
    
    logger.info(f"  ✅ Responses with citations: {len(citations_found)}/{len(truthfulqa_results.detailed_results)}")
    logger.info(f"  ⚠️  Responses without citations: {len(citations_missing)}/{len(truthfulqa_results.detailed_results)}")
    
    if citations_found:
        logger.info("")
        logger.info("  📝 Sample citations found:")
        for i, result in enumerate(citations_found[:3], 1):
            citation_text = result.metrics.get("citation_text", "Unknown")
            logger.info(f"    {i}. Q: {result.question[:50]}...")
            logger.info(f"       Citation: {citation_text}")
    
    if citations_missing:
        logger.info("")
        logger.info("  ⚠️  Questions missing citations:")
        for i, result in enumerate(citations_missing[:3], 1):
            logger.info(f"    {i}. {result.question[:60]}...")
            logger.info(f"       Response preview: {result.predicted_answer[:80]}...")
    logger.info("")
