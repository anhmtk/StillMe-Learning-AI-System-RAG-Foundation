...
def configure_power_settings():
    ...
    try:
        ...
        try:
            ...
            result = subprocess.run([
                "powercfg", "/duplicatescheme", "SCHEME_BALANCED", "StillMe-IPC-Learning"
            ], capture_output=True, text=True, check=True)
            
            # Extract GUID from result
            guid_line = [line for line in result.stdout.split('\n') if 'StillMe-IPC-Learning' in line]
            if guid_line:
        ...
        logger.info("💡 StillMe IPC will now continue learning even when display is off")
        logger.info("💡 System will not sleep, only display will turn off to save power")
...
def main():
    ...
    parser.add_argument("action", choices=["configure", "restore", "show"],
    ...
    if args.action == "configure":
        ...
        if success:
            ...
            print("💡 To restore defaults: python scripts/configure_power_settings.py restore")
        else:
