...
def check_comment_out():
    ...
    for hunk in hunks:
        ...
        has_deletions = any(line.startswith('-') and not line.startswith('---') for line in hunk)
        has_new_comments = any(
            line.startswith('+') and 
            line[1:].strip().startswith('#') and 
            not line[1:].strip().startswith('# type: ignore') and
            not line[1:].strip().startswith('# noqa') and
            not 'TEMP_DISABLE' in line
            for line in hunk
        )
        ...
        if has_deletions and has_new_comments:
            ...
            for line in hunk:
                if line.startswith('+') and line[1:].strip().startswith('#'):
                    ...
                    if any(keyword in comment_content.lower() for keyword in [
                        'license', 'copyright', 'author', 'version', 'date',
                        'docstring', 'module', 'function', 'class', 'method',
                        'todo', 'fixme', 'note', 'warning', 'info'
                    ]):
                        continue
                    
                    # Check for TEMP_DISABLE with valid expiration
                    if 'TEMP_DISABLE' in line:
                        temp_disable_match = re.search(r'TEMP_DISABLE\(reason=([^,]+),\s*expires=(\d{4}-\d{2}-\d{2})\)', line)
