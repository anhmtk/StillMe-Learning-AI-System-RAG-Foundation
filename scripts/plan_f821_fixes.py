...
def main():
    ...
    print(f"  ... and {len(plan['stdlib_fixes']) - 5} more" if len(plan['stdlib_fixes']) > 5 else "")
    print()

    print("🔄 CORE REEXPORTS (Priority 2):")
    for fix in plan["core_reexports"][:5]:
        print(f"  {fix['symbol']}: {fix['import']} ({fix['occurrences']} occurrences)")
    print(f"  ... and {len(plan['core_reexports']) - 5} more" if len(plan['core_reexports']) > 5 else "")
    print()
    ...
    print(f"  ... and {len(plan['missing_implementations']) - 5} more" if len(plan['missing_implementations']) > 5 else "")
    print()
