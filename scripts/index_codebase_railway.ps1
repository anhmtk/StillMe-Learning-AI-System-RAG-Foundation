...
if ([string]::IsNullOrEmpty($ApiKey)) {
    ...
    if ([string]::IsNullOrEmpty($ApiKey)) {
        Write-Host "API Key not provided!" -ForegroundColor Yellow
...
if (-not [string]::IsNullOrEmpty($ApiKey)) {
    ...
    Write-Host "No API key provided - request may fail if authentication is required" -ForegroundColor Yellow
}
...
try {
    ...
    if ($statsResponse.total_chunks -gt 0 -and -not $Force.IsPresent) {
        Write-Host ""
        Write-Host "Collection already has chunks!" -ForegroundColor Yellow
    ...
    Write-Host "Could not get stats: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
try {
    ...
    if ($status -eq "success") {
        ...
        if ($status -eq "skipped") {
            Write-Host "Indexing skipped: $($response.message)" -ForegroundColor Yellow
            Write-Host "Current count: $($response.current_count)" -ForegroundColor Yellow
        } else {
            $responseJson = $response | ConvertTo-Json -Depth 3
            Write-Host "Unexpected response: $responseJson" -ForegroundColor Yellow
        }
    }
