...
try:
    from stillme_core.safety_guard import apply_policies, safe_reply, redact_output  # type: ignore
except ImportError:
    ...
    for name in candidates:
        ...
        if callable(fn):
            ...
            for p in sig.parameters.values():
                if p.name in ("prompt", "text"): kwargs[p.name] = prompt
                elif p.name == "locale": kwargs["locale"] = locale
                elif p.name in ("safety", "safety_mode"): kwargs[p.name] = safety
            return fn(**kwargs)
    raise HTTPException(status_code=500, detail="No suitable framework method found")

def _normalize(result):
    blocked = bool(getattr(result, "blocked", False) if hasattr(result, "blocked") else result.get("blocked", False))
    text = str(getattr(result, "text", "") if hasattr(result, "text") else result.get("text", ""))
    reason = getattr(result, "reason", None) if hasattr(result, "reason") else result.get("reason")
...
def _inference_common(req: InferenceRequest):
    ...
    return {"blocked": blocked, "text": text, "reason": reason, "latency_ms": (time.perf_counter() - t0)*1000.0}

@app.post("/inference")
def inference(req: InferenceRequest): return _inference_common(req)

@app.post("/process")
def process(req: InferenceRequest): return _inference_common(req)