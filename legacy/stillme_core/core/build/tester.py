...
class LocalTester:
    ...
    def test_python_executable(
        self, exe_path: str, test_cases: List[Dict[str, Any]]
    ) -> TestSuite:
        """
        Test a Python executable
        ...
        logger.info(
            f"✅ Test suite completed: {suite.passed_tests}/{suite.total_tests} passed"
        )

        return suite

    def test_web_app(
        self, app_path: str, port: int = 5000, framework: str = "flask"
    ) -> TestSuite:
        """
        Test a web application
        ...
        logger.info(
            f"✅ Web app test completed: {suite.passed_tests}/{suite.total_tests} passed"
        )

        return suite

    def test_cli_tool(self, tool_path: str, test_commands: List[str]) -> TestSuite:
        """
        Test a CLI tool
        ...
        logger.info(
            f"✅ CLI tool test completed: {suite.passed_tests}/{suite.total_tests} passed"
        )
    ...
    def _test_custom_case(
        self, exe_path: str, test_case: Dict[str, Any], index: int
    ) -> TestResult:
        ...
        try:
            ...
            success = result.returncode == expected_exit_code and (
                not expected_output or expected_output in result.stdout
            )
    ...
    def _test_performance(self, exe_path: str) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                execution_time=execution_time,
                output=f"Average execution time: {avg_time:.3f}s",
                error_message=f"Performance too slow: {avg_time:.3f}s"
                if not success
                else None,
                test_type="performance",
            )
    ...
    def _test_web_startup(self, app_path: str, port: int, framework: str) -> TestResult:
        ...
        try:
            ...
            process = subprocess.Popen(  # type: ignore
                cmd, cwd=app_path, capture_output=True, text=True
            )
    ...
    def _test_web_performance(self, port: int) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                execution_time=execution_time,
                output=f"Average response time: {avg_response_time:.3f}s",
                error_message=f"Response too slow: {avg_response_time:.3f}s"
                if not success
                else None,
                test_type="web_performance",
            )
