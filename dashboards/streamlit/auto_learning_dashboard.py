...
class AutoLearningDashboard:
    ...
    def render_sidebar(self):
        ...
        if st.sidebar.button("🔄 Chạy Auto-Discovery Ngay"):
            try:
                with st.spinner("Đang chạy knowledge discovery..."):
                    ...
                    discovery_system = KnowledgeDiscoverySystem()
                    proposals_created = discovery_system.run_discovery_cycle()
                    st.success(
                        f"✅ Đã chạy auto-discovery - Tạo {proposals_created} proposals mới"
                    )
                    st.rerun()  # Refresh dashboard
            except Exception as e:
                st.error(f"❌ Lỗi chạy auto-discovery: {e}")
        ...
        if st.sidebar.button("🤖 Chạy Auto-Approval Ngay"):
            try:
                ...
                st.error(f"❌ Lỗi chạy auto-approval: {e}")
        ...
        if silent_sessions:
            for session in silent_sessions:
                with st.expander(f"🔇 {session['title']} - {session['progress']:.1f}%"):
                    ...
                    st.write(
                        f"**Current Objective:** {session['current_objective_index'] + 1}/{len(session['objectives'])}"
                    )
    ...
    def get_auto_approved_count(self):
        ...
        try:
            ...
            auto_approved = [
                p for p in proposals if p.get("approved_by") == "auto_approval_system"
            ]
    ...
    def get_silent_learning_sessions(self):
        ...
        try:
            ...
            for proposal in learning_sessions:
                session_data = self.learning_system.get_session_status(
    ...
    def get_completed_today(self):
        ...
        try:
            ...
            completed_today = [
                ...
                and p.get("learning_completed_at")
                and datetime.fromisoformat(p["learning_completed_at"]).date() == today
            ]
    ...
    def render_learning_progress_chart(self):
        ...
        proposals = self.proposals_manager.get_all_proposals()
        learning_proposals = [
            p for p in proposals if p.get("status") in ["learning", "completed"]
        ]
    ...
    def render_recent_completions(self):
        ...
        try:
            ...
            if completed:
                ...
                completed.sort(
                    key=lambda x: x.get("learning_completed_at", ""), reverse=True
                )
                ...
                for proposal in recent:
                    with st.container():
                        ...
                        st.write(
                            f"⏱️ Duration: {proposal.get('estimated_duration', 0)} minutes"
                        )
                        st.write(
                            f"📅 Completed: {proposal.get('learning_completed_at', 'Unknown')}"
                        )
                        st.divider()
            else:
    ...
    def render_completion_stats(self):
        ...
        try:
            ...
            fig = go.Figure(
                data=[go.Pie(labels=labels, values=values, marker_colors=colors)]
            )
