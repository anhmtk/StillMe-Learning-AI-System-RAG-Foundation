...
\textbf{Code Repository}: StillMe is fully open-source and available at \url{https://github.com/anhmtk/StillMe-Learning-AI-System-RAG-Foundation}. The system is deployed and operational, demonstrating practical deployability.
...
\item \textbf{Vector Database}: ChromaDB with collections \texttt{stillme\_knowledge} (learned content) and \texttt{stillme\_conversations} (conversation history)
...
\item \textbf{Visible Sources}: Users can see exactly what StillMe learns and from where through the dashboard and API endpoints (\texttt{GET /api/learning/sources/current}).
...
\item \textbf{Validation Logs}: All validation decisions are logged and visible through API endpoints (\texttt{GET /api/validators/metrics}).
\end{enumerate}
...
\begin{itemize}
\item \textbf{Code Repository}: \url{https://github.com/anhmtk/StillMe-Learning-AI-System-RAG-Foundation}
...
\begin{itemize}
\item \textbf{Evaluation Scripts}: \texttt{evaluation/comparison.py}, \texttt{scripts/run\_comparison\_only.py}, \texttt{scripts/run\_full\_evaluation.py}
...
\item API endpoints: \texttt{GET /api/learning/metrics/daily}, \texttt{GET /api/learning/metrics/range}
\end{itemize}
