...
class ChaosRunner:
    ...
    def test_process_kill_tolerance(self) -> bool:
        ...
        kill_tests = [
            ...
            {
                "name": "force_kill",
                "signal": getattr(signal, "SIGKILL", signal.SIGTERM),
        ...
        for test_config in kill_tests:
            ...
            try:
                ...
                if (
                    result["success"]
                    and result["recovery_time_ms"] <= test_config["expected_recovery"]
                ):
                    passed += 1
                    print(
                        f"PASSED: {test_config['name']} - {result['recovery_time_ms']:.1f}ms recovery"
                    )
                else:
                    print(
                        f"FAILED: {test_config['name']} - {result['recovery_time_ms']:.1f}ms recovery (expected {test_config['expected_recovery']}ms)"
                    )
        ...
        for test_config in delay_tests:
            ...
            try:
                result = self._simulate_network_delay(
                    test_config["delay_ms"], test_config["expected_recovery"]
                )
                ...
                if (
                    ...
                    passed += 1
                    print(
                        f"PASSED: {test_config['delay_ms']}ms delay - {result['recovery_time_ms']:.1f}ms recovery"
                    )
                else:
                    print(
                        f"FAILED: {test_config['delay_ms']}ms delay - {result['recovery_time_ms']:.1f}ms recovery (expected {test_config['expected_recovery']}ms)"
                    )
        ...
        for test_config in storage_tests:
            ...
            try:
                result = self._simulate_storage_drop(
                    test_config["name"], test_config["expected_recovery"]
                )
    ...
    def _simulate_process_kill(
        self, name: str, signal_type: int, expected_recovery: int
    ) -> dict[str, Any]:
    ...
    def _simulate_network_delay(
        self, delay_ms: int, expected_recovery: int
    ) -> dict[str, Any]:
    ...
    def _simulate_storage_drop(
        self, name: str, expected_recovery: int
    ) -> dict[str, Any]:
