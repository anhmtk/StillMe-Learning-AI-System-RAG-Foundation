...
async def main():
    ...
    try:
        ...
        print(f"🔧 Monthly improvement: {config.monthly_improvement_day}st at {config.monthly_improvement_time}")
        ...
        try:
            while True:
                ...
                if hasattr(main, 'last_status_time'):
                    if asyncio.get_event_loop().time() - main.last_status_time > 3600:  # 1 hour
                        status = scheduler.get_status()
                        stats = status['job_stats']
                        print(f"\n📊 Hourly Status Update:")
                        for job_id, job_stat in stats.items():
                            print(f"   - {job_id}: executed={job_stat['executed']}, failed={job_stat['failed']}")
